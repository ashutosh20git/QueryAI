...
import { createQueryAI } from "@queryai/sdk"
...
`)
  ...
  const queryai = await createQueryAI({ port: 0 })
  ...
  try {
    ...
    const session = await queryai.client.session.create()
    const result = await queryai.client.session
      .prompt({
