# Remove `packages/queryai/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/queryai/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the queryai legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/queryai/src/storage/db.ts` are concentrated in 22 source files:

- `packages/queryai/src/account/repo.ts`
- `packages/queryai/src/cli/cmd/db.ts`
- `packages/queryai/src/cli/cmd/import.ts`
- `packages/queryai/src/cli/cmd/stats.ts`
- `packages/queryai/src/control-plane/workspace.ts`
- `packages/queryai/src/index.ts`
- `packages/queryai/src/node.ts`
- `packages/queryai/src/permission/index.ts`
- `packages/queryai/src/project/project.ts`
- `packages/queryai/src/server/projectors.ts`
- `packages/queryai/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/queryai/src/server/shared/fence.ts`
- `packages/queryai/src/session/message-v2.ts`
- `packages/queryai/src/session/projectors.ts`
- `packages/queryai/src/session/prompt.ts`
- `packages/queryai/src/session/session.ts`
- `packages/queryai/src/session/todo.ts`
- `packages/queryai/src/share/share-next.ts`
- `packages/queryai/src/storage/db.ts`
- `packages/queryai/src/sync/index.ts`
- `packages/queryai/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy queryai database wrapper; `packages/queryai/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the queryai wrapper.
...
Status: Completed. `SyncEvent` and the queryai projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy queryai database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy queryai database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy queryai database wrapper.
...
All migration groups are complete or superseded. `packages/queryai/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No queryai data-migration group remains.

The previous queryai `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate queryai data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/queryai`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/queryai/src`
- `bun typecheck` from `packages/queryai`
- Relevant package tests from `packages/queryai`, not the repo root
