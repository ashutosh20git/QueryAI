...
The queryai server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@queryai/llm` tool loop after replacing its
...
- plugins get global "queryai" instance like in that post i showed
- queryai instance has stuff like `queryai.session.prompt()` or
  `queryai.tool.register({...})`
...
- decide whether to preserve deprecated `@queryai/llm` orchestration exports
