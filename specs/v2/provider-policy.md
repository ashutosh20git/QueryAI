...
Both `action` and `resource` use queryai's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.queryai` files is intentionally deferred until `.queryai` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether queryai permits an operation through its managed execution paths.
