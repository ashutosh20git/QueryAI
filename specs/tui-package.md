...
Move the canonical QueryAI terminal application from
`packages/queryai/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/queryai`,
`packages/cli`, or `@queryai/core`.

The SDK is the TUI's QueryAI boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/queryai`.
- Do not replace private `packages/queryai` imports with `@queryai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@queryai/tui`.
...
- QueryAI domain data displayed by the TUI
...
```text
packages/queryai/src/cli/cmd/tui
```

Its private dependency on `packages/queryai` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/queryai/src/*`.
...
Status: Completed. The private `@queryai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@queryai/tui`.
...
- The package has no dependency on `queryai`, `@queryai/cli`, or
  `@queryai/core`.
...
primitives now live in `@queryai/tui`. The legacy host consumes explicit
...
and build inputs through `@queryai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads QueryAI globals or process state;
...
Replace process-global QueryAI state with resolved TUI inputs.
...
`@queryai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/queryai` initially.
...
canonical in `@queryai/tui`. Configured references resolve through the new
...
adapters rather than QueryAI domain implementations.

Make the SDK the only QueryAI domain boundary used by the TUI.
...
- Domain-facing TUI code imports QueryAI data and operations only from
  `@queryai/sdk`.
...
`@queryai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/queryai`.
...
- The package has no imports from `packages/queryai`, `packages/cli`, or
  `@queryai/core`.
...
public `@queryai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/queryai`.
- Keep the legacy embedded worker and server startup in `packages/queryai`.
- Change those adapters to load config, create transport inputs, and call the
  public `@queryai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/queryai`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@queryai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@queryai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for QueryAI domain state.
...
cd packages/queryai && bun typecheck
...
rg '@queryai/core|packages/queryai|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/queryai/src packages/queryai/test
```
...
- Build the current-platform `packages/queryai` binary.
