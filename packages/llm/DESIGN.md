...
> current private `@queryai/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@queryai/ai`
...
- Secondary audience: QueryAI and other durable agent runtimes
...
import { LLM } from "@queryai/ai"
import { OpenAI } from "@queryai/ai/providers/openai"
...
```ts
import { Message } from "@queryai/ai"
...
import { LLM, Tool } from "@queryai/ai"
...
QueryAI and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@queryai/ai/promise"
import { OpenAI } from "@queryai/ai/providers/openai"
...
```ts
import { LLMSchema } from "@queryai/ai/schema"
...
```ts
import { Provider, Protocol } from "@queryai/ai/provider"
...
@queryai/ai/providers/openai
@queryai/ai/providers/anthropic
@queryai/ai/providers/google
...
```
...
QueryAI should migrate to `generateTurn` / `streamTurn`, preserving its durable
