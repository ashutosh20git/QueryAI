...
This is the living design and status document for `@queryai/codemode` and its existing V2 QueryAI adapter.
...
`packages/queryai/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
