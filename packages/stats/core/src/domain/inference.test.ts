...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve queryai route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "queryai")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "queryai")).toBe("openai")
    expect(statProvider("big-pickle", "", "queryai")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "queryai"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep queryai as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "queryai"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "queryai"))).toMatchObject([{ provider: "unknown" }])
    expect(toProviderAggregate(aggregate("muse-spark-1.2-contributor", "unknown"))).toMatchObject([
      { provider: "meta" },
    ])
  })

  test("geo aggregates never keep queryai or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "queryai"), country: "US" })).toMatchObject([
