...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall queryai and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall QueryAI")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: `npm uninstall -g ${Installation.Release.npm}`,
      pnpm: `pnpm uninstall -g ${Installation.Release.npm}`,
      bun: `bun remove -g ${Installation.Release.npm}`,
      yarn: `yarn global remove ${Installation.Release.npm}`,
      brew: "brew uninstall queryai",
      choco: "choco uninstall queryai",
      scoop: "scoop uninstall queryai",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "queryai", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using QueryAI!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# queryai") || content.includes(".queryai/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".queryai/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".queryai/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".queryai"))
    ) {
      continue
    }
