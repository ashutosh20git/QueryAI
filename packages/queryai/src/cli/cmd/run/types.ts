// Shared type vocabulary for the direct interactive mode (`queryai --mini`).
//
...
import type { QueryAIClient, PermissionRequest, QuestionRequest, ToolPart } from "@queryai/sdk/v2"
import type { TuiConfig } from "@queryai/tui/config"
...
type PromptModel = Parameters<QueryAIClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<QueryAIClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<QueryAIClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<QueryAIClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<QueryAIClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<QueryAIClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<QueryAIClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<QueryAIClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<QueryAIClient["question"]["reject"]>[0]
