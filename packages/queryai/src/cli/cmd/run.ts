import type { PermissionV1 } from "@queryai/core/v1/permission"
import { FSUtil } from "@queryai/core/fs-util"
// CLI entry point for `queryai run` and `queryai --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`queryai --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`queryai --mini --attach`): connects to a running
//      queryai server and runs interactive mode against it.
//
...
import { createQueryAIClient, type QueryAIClient, type ToolPart } from "@queryai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<QueryAIClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run queryai with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running queryai server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to QUERYAI_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to QUERYAI_SERVER_USERNAME or 'queryai')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: QueryAIClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: QueryAIClient, sessionID: string) {
      ...
      async function current(sdk: QueryAIClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: QueryAIClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: QueryAIClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: QueryAIClient) {
        ...
        async function loop(client: QueryAIClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
