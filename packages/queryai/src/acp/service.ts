...
import type { AssistantMessage, Message, QueryAIClient, SessionMessageResponse } from "@queryai/sdk/v2"
...
export const AuthMethodID = "queryai-login"
...
export class Service extends Context.Service<Service, Interface>()("@queryai/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `queryai auth login` in the terminal",
      name: "Login with queryai",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: QueryAIClient) {
...
function makeUsageService(sdk: QueryAIClient) {
...
async function loadDirectorySnapshot(sdk: QueryAIClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, queryai provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const queryaiProvider = providers[ProviderV2.ID.make("queryai")]
  const queryaiModel = queryaiProvider ? Provider.sort(Object.values(queryaiProvider.models))[0] : undefined
  if (queryaiProvider && queryaiModel) return { providerID: queryaiProvider.id, modelID: queryaiModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "QueryAI service failure", service })
}
