...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@queryai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@queryai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@queryai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@queryai/tui/context/theme"
import { Global } from "@queryai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@queryai/core/util/flock"
import { Flag } from "@queryai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@queryai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@queryai/core/v1/config/plugin"
import { createCommandShim } from "@queryai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@queryai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".queryai"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".queryai", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".queryai", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.QUERYAI_PURE ? [] : pluginOrigins
    if (Flag.QUERYAI_PURE && pluginOrigins.length) {
    }
