...
export async function checkPluginCompatibility(target: string, queryaiVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(queryaiVersion) || semver.major(queryaiVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.queryai
  if (typeof range !== "string") return
  if (!semver.satisfies(queryaiVersion, range)) {
    throw new Error(`Plugin requires queryai ${range} but running ${queryaiVersion}`)
  }
}
