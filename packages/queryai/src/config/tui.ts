...
import { isRecord } from "@queryai/tui/util/record"
...
import { TuiKeybind } from "@queryai/tui/config/keybind"
import { InstallationLocal, InstallationVersion } from "@queryai/core/installation/version"
import { makeRuntime } from "@queryai/core/effect/runtime"
import { Filesystem } from "@/util/filesystem"
import { ConfigVariable } from "@/config/variable"
import { Npm } from "@queryai/core/npm"
import { FormatError, FormatUnknownError } from "@/cli/error"
import { TuiConfig } from "@queryai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@queryai/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old queryai.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.queryai`
  // folders between cwd and home, and QUERYAI_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.QUERYAI_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit QUERYAI_TUI_CONFIG override, if set.
  if (Flag.QUERYAI_TUI_CONFIG) {
    const configFile = Flag.QUERYAI_TUI_CONFIG
  ...
  // 4. `.queryai` directories (and QUERYAI_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".queryai") || dir === Flag.QUERYAI_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".queryai") && dir !== Flag.QUERYAI_CONFIG_DIR) continue
