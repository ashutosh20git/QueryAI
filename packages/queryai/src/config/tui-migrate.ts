...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from queryai.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const queryai = await queryaiFiles(input)
  for (const file of queryai) {
    ...
    // No `$schema`: there is no published tui.json schema to point at, and a
    // dead URL in every migrated config is worse than none.
...
async function queryaiFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "queryai"),
    ...(await Filesystem.findUp(["queryai.json", "queryai.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "queryai"))
  }
  if (Flag.QUERYAI_CONFIG) files.push(Flag.QUERYAI_CONFIG)
