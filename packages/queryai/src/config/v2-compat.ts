...
import { NonNegativeInt, PositiveInt } from "@queryai/core/schema"
import { ConfigAttachmentV1 } from "@queryai/core/v1/config/attachment"
import { ConfigLSPV1 } from "@queryai/core/v1/config/lsp"
import { InvalidError } from "@queryai/core/v1/config/error"
  ...
  if (permissions.length)
    throw new InvalidError({
      ...
      issues: permissions.map((path) => ({
        path,
        message: 'V2 permissions are not supported by QueryAI V1. Use V1 "permission" rules or run queryai2.',
      })),
    })
