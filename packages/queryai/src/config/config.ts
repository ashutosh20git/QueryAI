...
import { InstallationLocal, InstallationVersion } from "@queryai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@queryai/core/v1/config/console-state"
import { FSUtil } from "@queryai/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@queryai/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@queryai/core/v1/config/config"
import { RemoteAuthError } from "@queryai/core/v1/config/error"
import { ConfigPermissionV1 } from "@queryai/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@queryai/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["queryai.jsonc", "queryai.json", "config.json"].map((file) => path.join(Global.Path.config, file))
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = ConfigV1.SCHEMA_URL
        const updated = text.replace(/^\s*\{/, `{\n  "$schema": "${ConfigV1.SCHEMA_URL}",`)
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.QUERYAI_CONFIG && !Flag.QUERYAI_CONFIG_DIR && !Flag.QUERYAI_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: ConfigV1.SCHEMA_URL }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "queryai.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "queryai.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = ConfigV1.SCHEMA_URL
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "QUERYAI_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/queryai`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = ConfigV1.SCHEMA_URL
        ...
        if (Flag.QUERYAI_CONFIG) {
          yield* merge(Flag.QUERYAI_CONFIG, yield* loadFile(Flag.QUERYAI_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.QUERYAI_CONFIG })
        }

        if (!Flag.QUERYAI_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("queryai", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.QUERYAI_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from QUERYAI_CONFIG_DIR", { path: Flag.QUERYAI_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".queryai") || dir === Flag.QUERYAI_CONFIG_DIR) {
            for (const file of ["queryai.json", "queryai.jsonc"]) {
          ...
          // Auto-discovered plugins under `.queryai/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.QUERYAI_CONFIG_CONTENT) {
          const source = "QUERYAI_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.QUERYAI_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from QUERYAI_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["QUERYAI_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("QUERYAI_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.QUERYAI_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.QUERYAI_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("QUERYAI_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.QUERYAI_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.QUERYAI_DISABLE_PRUNE) {
