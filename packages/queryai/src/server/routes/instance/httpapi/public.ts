...
import { QueryAIHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for queryai serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(QueryAIHttpApi)
...
export const PublicApi = QueryAIHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "queryai",
    version: "1.0.0",
    description: "queryai api",
