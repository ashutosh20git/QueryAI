...
import { McpEvent } from "@queryai/schema/mcp-event"
import { McpBrowser } from "./browser"

const DEFAULT_TIMEOUT = 30_000
const CLIENT_OPTIONS = {
  capabilities: {
    // sampling, elicitation and tasks stay off: each needs client-side support
    // that is not implemented yet, and advertising a capability we cannot serve
    // makes servers take paths that then fail.
    // sampling: {},
    // elicitation: {},
    roots: {},
    // tasks: {},
...
function createClient(directory: string) {
  const client = new Client({ name: "queryai", version: InstallationVersion }, CLIENT_OPTIONS)
...
export class Service extends Context.Service<Service, Interface>()("@queryai/MCP") {}
            ...
            if (isAuthError) {
              if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                ...
                return events
                  .publish(TuiEvent.ToastShow, {
                    title: "MCP Authentication Required",
                    message: `Server "${key}" requires authentication. Run: queryai mcp auth ${key}`,
                    variant: "warning",
                    duration: 8000,
                  })
                  .pipe(Effect.ignore, Effect.as(undefined))
