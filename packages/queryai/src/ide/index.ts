...
import { IdeEvent } from "@queryai/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["QUERYAI_CALLER"] === "vscode" || process.env["QUERYAI_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.queryai"], {
    nothrow: true,
  })
