...
import type { LLMClientShape } from "@queryai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("queryai"))
    return { type: "unsupported", reason: "provider is not openai, queryai, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @queryai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains queryai-owned. The native runtime only adapts
      // the @queryai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
