...
Trim `packages/queryai/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/queryai/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/queryai`.
- `[x]` `bun typecheck` from `packages/queryai`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/queryai/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/queryai`.
- `[x]` `bun dev generate > /tmp/queryai-openapi.json` from `packages/queryai`.
...
- Edit `packages/queryai/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/queryai-openapi.json` from `packages/queryai`.
...
- Check source schemas in files like `packages/queryai/src/session/schema.ts`, `packages/queryai/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/queryai/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/queryai/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
