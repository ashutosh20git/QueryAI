...
- Author package entrypoint is `@queryai/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/queryai-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@queryai/plugin/tui`.
- `@queryai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@queryai/plugin/tui"
...
- The value is a semver range checked against the running QueryAI version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.queryai` is absent, no check is performed (backward compatible).
...
- `queryai plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `queryai plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.queryai` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.queryai`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.queryai`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.queryai` dir or `QUERYAI_CONFIG_DIR`, QueryAI installs `@queryai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@queryai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus QueryAI's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
QueryAI registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, QueryAI pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@queryai/plugin/tui`.
...
- `message` is required; `title` defaults to `"queryai"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `queryai.default`.
...
- Local plugins persist installed themes under the local `.queryai/themes` area near the plugin config source.
...
- `--pure` / `QUERYAI_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.queryai/plugins/tui-smoke.tsx`
- Local vim plugin: `.queryai/plugins/tui-vim.tsx`
- Local smoke config: `.queryai/tui.json`
- Local smoke theme: `.queryai/plugins/smoke-theme.json`
