...
Delete `packages/queryai/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/queryai/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/queryai/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/queryai`
- TUI plugin loader tests in `packages/queryai` if runtime plugin API wiring changed
