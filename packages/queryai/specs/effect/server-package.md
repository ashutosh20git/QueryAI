...
Practical reference for a future `packages/server` split after the queryai
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/queryai`.
...
Until enough shared service code lives outside `packages/queryai`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/queryai`

It should not import `packages/queryai` services while `packages/queryai`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on queryai-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/queryai` implementation details.
