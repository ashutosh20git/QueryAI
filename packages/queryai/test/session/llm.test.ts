...
import { LLMClient, RequestExecutor } from "@queryai/llm/route"
...
describe("session.llm.stream", () => {
  ...
  const queryaiFixture = { providerID: "queryai-test", modelID: vivgridFixture.modelID }

  it.instance(
    "sends the parent session header for queryai providers",
    () =>
      Effect.gen(function* () {
        ...
        const resolved = yield* Provider.use.getModel(
          ProviderV2.ID.make(queryaiFixture.providerID),
          ModelV2.ID.make(queryaiFixture.modelID),
        )
        ...
        const user = {
          ...
          agent: agent.name,
          model: {
            providerID: ProviderV2.ID.make(queryaiFixture.providerID),
            modelID: resolved.id,
          },
      ...
      }),
    {
      config: () => {
        const fixture = loadFixture(vivgridFixture.providerID, vivgridFixture.modelID)
        return {
          enabled_providers: [queryaiFixture.providerID],
          provider: {
            [queryaiFixture.providerID]: {
              name: "QueryAI Test",
