...
import { LLMEvent, ToolFailure } from "@queryai/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@queryai/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("queryai") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("queryai") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, queryai, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored queryai auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("queryai") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("queryai"),
