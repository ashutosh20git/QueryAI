...
import { HttpRecorder } from "@queryai/http-recorder"
import { HttpRecorderInternal } from "@queryai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@queryai/llm"
import { RequestExecutor } from "@queryai/llm/route"
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.QUERYAI_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["queryai", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("QUERYAI_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("QUERYAI_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["queryai", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "queryai-proxy",
    name: "QueryAI proxy",
    providerID: ProviderV2.ID.queryai,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["queryai", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.QUERYAI_RECORD_CONSOLE_TOKEN && process.env.QUERYAI_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.queryai,
        name: "QueryAI Zen",
        env: ["QUERYAI_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.QUERYAI_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.QUERYAI_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.QUERYAI_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["queryai", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("QUERYAI_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("QUERYAI_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("QUERYAI_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set QUERYAI_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the queryai LLM stack remain real.
  const metadata = {
