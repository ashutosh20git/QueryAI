...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ queryai }) =>
    Effect.gen(function* () {
      const result = yield* queryai.spawn(["--replay-limit", "10"])

      queryai.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ queryai }) =>
    Effect.gen(function* () {
      const result = yield* queryai.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ queryai }) =>
    Effect.gen(function* () {
      const result = yield* queryai.spawn(["--mini", "--port", "4096"])
