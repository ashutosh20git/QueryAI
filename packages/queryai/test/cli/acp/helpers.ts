...
export function createAcpClient(input: Pick<CliFixture, "queryai">, env?: Record<string, string>) {
  return Effect.gen(function* () {
    return createJsonRpcAcpClient(yield* input.queryai.acp(env ? { env } : undefined))
  })
}
  ...
  return Effect.gen(function* () {
    return expectOk(
      yield* acp.request<InitializeResponse>("initialize", {
        ...
        clientInfo: { name: "queryai-local-acp", version: "0.1.0" },
      }),
    )
  })
}
