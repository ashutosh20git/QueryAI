...
describe("queryai acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ queryai }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ queryai }))
        ...
        expect(initialized.agentInfo?.name).toBe("QueryAI")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ queryai }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ queryai })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("queryai-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "queryai-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.QUERYAI_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ queryai }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("queryai-login")
