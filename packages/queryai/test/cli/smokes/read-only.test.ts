...
describe("queryai read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // QUERYAI_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ queryai }) =>
      Effect.gen(function* () {
        const r = yield* queryai.spawn(["mcp", "list"])
        queryai.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ queryai }) =>
      Effect.gen(function* () {
        const r = yield* queryai.spawn(["providers", "list"])
        queryai.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ queryai }) =>
      Effect.gen(function* () {
        const r = yield* queryai.spawn(["models"])
        queryai.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ queryai }) =>
      Effect.gen(function* () {
        const r = yield* queryai.spawn(["agent", "list"])
        queryai.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh QUERYAI_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ queryai }) =>
      Effect.gen(function* () {
        const r = yield* queryai.spawn(["session", "list"])
        queryai.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ queryai }) =>
      Effect.gen(function* () {
        const r = yield* queryai.spawn(["stats"])
        queryai.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under QUERYAI_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ queryai }) =>
      Effect.gen(function* () {
        const r = yield* queryai.spawn(["db", "path"])
        queryai.expectExit(r, 0, "db path")
