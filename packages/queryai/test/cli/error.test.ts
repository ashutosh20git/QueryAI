...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/queryai.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/queryai.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/queryai.jsonc", dir: ".queryai", suggestion: "queryai" },
        expected:
          'Directory ".queryai" in /tmp/queryai.jsonc is not valid. Rename the directory to "queryai" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/queryai.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/queryai.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `queryai models` to list available models",
      "Or check your config (queryai.json) provider/model names",
    ].join("\n")
