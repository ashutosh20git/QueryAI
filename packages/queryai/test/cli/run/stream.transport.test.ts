...
import { QueryAIClient, type GlobalEvent } from "@queryai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<QueryAIClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<QueryAIClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<QueryAIClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<QueryAIClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<QueryAIClient["session"]["status"]>>["data"]>
...
function sdk(
  input: {
    ...
    subscribe?: QueryAIClient["event"]["subscribe"]
    globalEvent?: QueryAIClient["global"]["event"]
    promptAsync?: QueryAIClient["session"]["promptAsync"]
    status?: QueryAIClient["session"]["status"]
    messages?: QueryAIClient["session"]["messages"]
    children?: QueryAIClient["session"]["children"]
    permissions?: QueryAIClient["permission"]["list"]
    questions?: QueryAIClient["question"]["list"]
  } = {},
) {
  const client = new QueryAIClient()

  const subscribe: QueryAIClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: QueryAIClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: QueryAIClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: QueryAIClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: QueryAIClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: QueryAIClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: QueryAIClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: QueryAIClient["question"]["list"] = input.questions ?? (() => ok([]))
