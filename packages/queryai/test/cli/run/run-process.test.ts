// Subprocess integration tests for `queryai run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `queryai.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `QUERYAI_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("queryai run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, queryai }) =>
      Effect.gen(function* () {
        ...
        const result = yield* queryai.run("say hi")
        queryai.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, queryai }) =>
      Effect.gen(function* () {
        ...
        const result = yield* queryai.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, queryai }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* queryai.run("think", { extraArgs: ["--thinking"] })
        queryai.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* queryai.run("think again")
        queryai.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, queryai }) =>
      Effect.gen(function* () {
        ...
        const result = yield* queryai.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, queryai }) =>
      Effect.gen(function* () {
        ...
        const result = yield* queryai.run("say hi", { format: "json" })
        queryai.expectExit(result, 0)

        const events = queryai.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ queryai }) =>
      Effect.gen(function* () {
        const result = yield* queryai.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, queryai }) =>
      Effect.gen(function* () {
        ...
        const result = yield* queryai.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, queryai }) =>
      Effect.gen(function* () {
        ...
        yield* llm.text("recovered")
        const result = yield* queryai.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, queryai }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* queryai.run("request permission", { permission: { bash: "ask" } })
        queryai.expectExit(denied, 0)
        ...
        const allowed = yield* queryai.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        queryai.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* queryai.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        queryai.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, queryai }) =>
      Effect.gen(function* () {
        ...
        const server = yield* queryai.serve()

        const result = yield* queryai.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, queryai }) =>
      Effect.gen(function* () {
        const result = yield* queryai.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, queryai }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* queryai.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
