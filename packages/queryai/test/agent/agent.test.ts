...
it.instance("plan agent denies edits except .queryai/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".queryai/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".queryai", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.QUERYAI_TEST_HOME
      process.env.QUERYAI_TEST_HOME = test.directory
