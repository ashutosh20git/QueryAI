...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-queryai-directory", directory)
    ...
    Effect.gen(function* () {
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          body: JSON.stringify({ $schema: "https://raw.githubusercontent.com/ashutosh20git/QueryAI-dist/schema/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-queryai-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-queryai-workspace")
