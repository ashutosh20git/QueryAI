...
/**
 * Scoped override for `Flag.QUERYAI_WORKSPACE_ID`. Saves the previous value
...
export function withFixedWorkspaceID(id: WorkspaceV2.ID): Effect.Effect<void, never, Scope.Scope> {
  return Effect.gen(function* () {
    const previous = Flag.QUERYAI_WORKSPACE_ID
    Flag.QUERYAI_WORKSPACE_ID = id
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        Flag.QUERYAI_WORKSPACE_ID = previous
      }),
    )
  })
}
