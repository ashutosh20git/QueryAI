...
import { makeGlobalNode } from "@queryai/core/effect/app-node"
...
import { Installation, Release } from "../../src/installation"
import { InstallationChannel } from "@queryai/core/installation/version"
  ...
  describe("latest", () => {
    ...
    testEffect(
      ...
      Effect.gen(function* () {
        ...
        expect(npmCalls).toContain(`https://registry.npmjs.org/${Release.npm}/${InstallationChannel}`)
      }),
    )
    ...
    testEffect(
      ...
      Effect.gen(function* () {
        ...
        expect(bunCalls).toContain(`https://registry.npmjs.org/${Release.npm}/${InstallationChannel}`)
      }),
    )
    ...
    testEffect(
      ...
      Effect.gen(function* () {
        ...
        expect(pnpmCalls).toContain(`https://registry.npmjs.org/${Release.npm}/${InstallationChannel}`)
      }),
    )
    ...
    testEffect(
      testLayer(
        ...
        (cmd, args) => {
          ...
          if (cmd === "brew" && args.includes("--formula") && args.includes(Release.brewTap)) return ""
          if (cmd === "brew" && args.includes("--formula") && args.includes(Release.brewFormula))
            return Release.brewFormula
          return ""
        },
      ),
    ...
    testEffect(
      testLayer(
        ...
        (cmd, args) => {
          if (cmd === "brew" && args.includes(Release.brewTap) && args.includes("--formula")) return Release.brewFormula
