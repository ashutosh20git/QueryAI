// Subprocess test harness for the queryai CLI. Spawns the real binary against
...
//
// Configuration flows through queryai's built-in test affordances:
//   - QUERYAI_CONFIG_CONTENT      : provider config inline, no files to find
//   - QUERYAI_TEST_HOME           : pins os.homedir() → tmpdir
//   - QUERYAI_DISABLE_PROJECT_CONFIG : skip walking up for queryai.json
//   - QUERYAI_PURE                : skip external plugin discovery + install
//   - QUERYAI_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `queryai.run` is fully wired. The shape supports adding more
// builders (`queryai.serve(opts)`, `queryai.acp(opts)`, `queryai.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of QueryAICli.
...
import { AppProcess } from "@queryai/core/process"
...
const queryaiRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(queryaiRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    QUERYAI_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    QUERYAI_CONFIG_CONTENT: configJson,
    QUERYAI_DISABLE_PROJECT_CONFIG: "1",
    QUERYAI_PURE: "1",
    QUERYAI_DISABLE_AUTOUPDATE: "1",
    QUERYAI_DISABLE_AUTOCOMPACT: "1",
    QUERYAI_DISABLE_MODELS_FETCH: "1",
    QUERYAI_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `queryai run`. New flags should
...
// `queryai serve` is a long-lived process — it never exits on its own.
...
// `queryai acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type QueryAICli = {
  ...
  // Spawn `queryai serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `queryai acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("queryai.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          QUERYAI_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("queryai.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("queryai.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "queryai server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `queryai serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("queryai.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const queryai: QueryAICli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, queryai })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `queryai.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "queryai") {
...
// every call site. Use this for any test that needs the queryai CLI fixture.
//
...
// resources (e.g. `queryai.serve`) without an extra `Effect.scoped` wrapper —
