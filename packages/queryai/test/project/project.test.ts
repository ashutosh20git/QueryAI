...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const queryaiFile = path.join(tmp, ".git", "queryai")
      expect(yield* Effect.promise(() => Bun.file(queryaiFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "queryai")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "queryai")
      const wrongCache = path.join(parentDir, ".git", "queryai")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "queryai")
      const cacheB = path.join(bareB, "queryai")
      const wrongCache = path.join(parentDir, ".git", "queryai")
