...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "queryai.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "queryai.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "queryai.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "queryai.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["queryai.json", "queryai.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "queryai.json"),
        path.join(project, "queryai.jsonc"),
      ])
    })
  })
