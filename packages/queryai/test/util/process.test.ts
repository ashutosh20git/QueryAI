...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.QUERYAI_TEST ?? "")'), {
      env: {
        QUERYAI_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "QUERYAI_TEST_SHELL"], {
      shell: true,
      env: {
        QUERYAI_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("QUERYAI_TEST_SHELL=ok")
  })
