...
it.instance("updates config and preserves empty shell sentinel", () =>
  Effect.gen(function* () {
    ...
    yield* writeConfigEffect(
      test.directory,
      { $schema: "https://raw.githubusercontent.com/QueryAI-org/QueryAI/schema/config.json", shell: "bash" },
      "config.json",
    )
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({ $schema: "https://raw.githubusercontent.com/QueryAI-org/QueryAI/schema/config.json" }),
      ),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://raw.githubusercontent.com/QueryAI-org/QueryAI/schema/config.json")
})
