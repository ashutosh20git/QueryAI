...
import type { Event, QueryAIClient } from "@queryai/sdk/v2"
...
type PermissionReplyParams = Parameters<QueryAIClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "queryai-acp-permission-"))
