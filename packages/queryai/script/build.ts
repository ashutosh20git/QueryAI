...
const createEmbeddedWebUIBundle = async () => {
  ...
  await $`QUERYAI_CHANNEL=${Script.channel} bun run --cwd ${appDir} build`
...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    compile: {
      ...
      outfile: `dist/${name}/bin/queryai`,
      execArgv: [`--user-agent=queryai/${Script.version}`, "--use-system-ca", "--"],
      windows: {},
    },
    files: {
      [treeSitterWorkerPath]: treeSitterWorker,
      ...(embeddedFileMap ? { "queryai-web-ui.gen.ts": embeddedFileMap } : {}),
    },
    entrypoints: [
      "./src/index.ts",
      workerPath,
      treeSitterWorkerPath,
      ...(embeddedFileMap ? ["queryai-web-ui.gen.ts"] : []),
    ],
    define: {
      ...
      QUERYAI_WORKER_PATH: workerPath,
      QUERYAI_CHANNEL: `'${Script.channel}'`,
      QUERYAI_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "",
  ...
  if (item.os === process.platform && item.arch === process.arch && !item.abi) {
    const binaryPath = `dist/${name}/bin/queryai`
