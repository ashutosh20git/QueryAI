...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("queryai-find")
  api.delete("queryai-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("queryai-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("queryai-find", new Highlight(...rest))
    return true
  }
