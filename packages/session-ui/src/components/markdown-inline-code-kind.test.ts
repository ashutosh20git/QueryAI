...
describe("inlineCodeKind", () => {
  test("leaves code expressions as normal inline code", () => {
    ...
    expect(inlineCodeKind(`@queryai/app <StatusPopover />)`)).toBeUndefined()
  ...
  test("detects file and directory paths", () => {
    ...
    expect(inlineCodeKind(`~/.config/queryai`)).toBe("path")
    expect(inlineCodeKind(`@queryai/app`)).toBe("path")
  ...
  test("detects urls", () => {
    ...
    expect(inlineCodeKind(`file:///tmp/queryai`)).toBeUndefined()
