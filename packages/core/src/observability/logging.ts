...
export function fileLogger(file = path.join(Global.Path.log, "queryai.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = process.env.QUERYAI_LOG_LEVEL?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.QUERYAI_PRINT_LOGS === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
