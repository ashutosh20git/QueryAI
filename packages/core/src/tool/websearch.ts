...
import { ToolFailure } from "@queryai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@queryai/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.QUERYAI_WEBSEARCH_PROVIDER === "exa" || process.env.QUERYAI_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.QUERYAI_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("QUERYAI_EXPERIMENTAL") || truthy("QUERYAI_ENABLE_EXA") || truthy("QUERYAI_EXPERIMENTAL_EXA"),
    enableParallel: truthy("QUERYAI_ENABLE_PARALLEL") || truthy("QUERYAI_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `queryai/${InstallationVersion}`,
