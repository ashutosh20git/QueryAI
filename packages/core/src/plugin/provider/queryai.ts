...
import type { IntegrationOAuthMethodRegistration } from "@queryai/plugin/v2/effect/integration"
import { define } from "@queryai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@queryai/sdk/v2/types"
...
export const QueryAIPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "queryai",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("QueryAIPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("queryai")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load QueryAI provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("queryai", (integration) => {
        integration.name = "QueryAI"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "queryai", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("queryai")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("queryai")
      ...
      const item = catalog.provider.get(ProviderV2.ID.queryai)
      if (!item) return
      const hasKey = Boolean(process.env.QUERYAI_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("queryai")),
