...
export const QUERYAI_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const QUERYAI_MODE_KEY = "queryai.mode"

export const QueryAIKeymapProvider = KeymapProvider
export const useQueryAIKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type QueryAIModeStack = ReturnType<typeof createQueryAIModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, QueryAIModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createQueryAIModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(QUERYAI_MODE_KEY, QUERYAI_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(QUERYAI_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(QUERYAI_MODE_KEY, stack.at(-1)?.mode ?? QUERYAI_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? QUERYAI_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(QUERYAI_MODE_KEY, undefined)
...
export function useQueryAIModeStack() {
  return getQueryAIModeStack(useQueryAIKeymap())
}

export function getQueryAIModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("QueryAI mode stack is not registered for this keymap")
  return value
}
...
export function registerQueryAIKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createQueryAIModeStack(keymap)
