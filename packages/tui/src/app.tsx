...
import { registerQueryAISpinner } from "./component/register-spinner"
import { createDefaultOpenTuiKeymap } from "@opentui/keymap/opentui"
import { Deferred, Effect } from "effect"
import { Global } from "@queryai/core/global"
import { Flag } from "@queryai/core/flag/flag"
import { InstallationVersion } from "@queryai/core/installation/version"
...
import {
  ...
  QUERYAI_BASE_MODE,
  QueryAIKeymapProvider,
  registerQueryAIKeymap,
  useBindings,
  useQueryAIKeymap,
} from "./keymap"
  ...
  const result = yield* Effect.scoped(
    Effect.gen(function* () {
        ...
        Effect.tryPromise({
          try: () =>
            createCliRenderer({
              ...
              useMouse: !Flag.QUERYAI_DISABLE_MOUSE && input.config.mouse,
              consoleOptions: {
      ...
      yield* Effect.acquireRelease(
        Effect.sync(() => registerQueryAIKeymap(keymap, renderer, input.config)),
                ...
                <ErrorBoundary fallback={(error, reset) => <ErrorComponent error={error} reset={reset} mode={mode} />}>
                  ...
                  >
                    ...
                    >
                      <TuiStartupProvider
                        value={{
                          initialRoute: process.env.QUERYAI_ROUTE ? JSON.parse(process.env.QUERYAI_ROUTE) : undefined,
                          skipInitialLoading: Boolean(process.env.QUERYAI_FAST_BOOT),
                        }}
                      >
                        <ClipboardProvider>
                          <QueryAIKeymapProvider keymap={keymap}>
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  const offSelectionKeys = keymap.intercept(
    "key",
    ({ event }) => {
      if (!Flag.QUERYAI_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.QUERYAI_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("QueryAI")
      return
    }
    ...
    if (route.data.type === "session") {
      ...
      renderer.setTerminalTitle(`QueryAI | ${title}`)
      return
    }

    if (route.data.type === "plugin") {
      renderer.setTerminalTitle(`QueryAI | ${route.data.id}`)
    }
  })
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        category: "Workspace",
        hidden: !Flag.QUERYAI_EXPERIMENTAL_WORKSPACES,
      ...
      },
      {
        ...
        title: "Open docs",
        run: () => {
          open("https://github.com/QueryAI-org/QueryAI#readme").catch(() => {})
          dialog.clear()
        },
        category: "System",
      },
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to QueryAI v${result.data.version}. Please restart the application.`,
    )
  ...
  return (
    <box
      ...
      onMouseUp={
        !Flag.QUERYAI_EXPERIMENTAL_DISABLE_COPY_ON_SELECT ? () => Selection.copy(renderer, toast, clipboard) : undefined
      }
    >
      <Show when={Flag.QUERYAI_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
