import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@queryai/plugin/tui"
...
import type { useQueryAIKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useQueryAIKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getQueryAIModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getQueryAIModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
