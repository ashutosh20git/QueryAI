...
import { getQueryAIModeStack, QUERYAI_BASE_MODE, QueryAIKeymapProvider, registerQueryAIKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerQueryAIKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when queryai mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getQueryAIModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getQueryAIModeStack(keymap).push("autocomplete")
