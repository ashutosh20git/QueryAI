...
const DEFAULT_SERVER_URL_KEY = "queryai.settings.dat:defaultServerUrl"
...
const getCurrentUrl = () => {
  if (import.meta.env.DEV)
    return `http://${import.meta.env.VITE_QUERYAI_SERVER_HOST ?? "localhost"}:${import.meta.env.VITE_QUERYAI_SERVER_PORT ?? "4096"}`
  ...
  Sentry.init({
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) => i.name !== "Breadcrumbs" && !(import.meta.env.QUERYAI_CHANNEL === "prod" && i.name === "GlobalHandlers"),
      )
    },
  })
}
