...
import { wslQueryAIAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.queryaiChecks[item.config.distro]
          const queryaiAction = () => wslQueryAIAction(check())
          const busy = () => wsl.data?.job?.kind === "install-queryai" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={queryaiAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installQueryAI(item.config.distro))}
                    >
