...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-queryai" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslQueryAIAction(check?: WslQueryAICheck) {
  if (!check) return
  if (!check.resolvedPath) return "wsl.onboarding.installQueryAI"
  if (check.matchesDesktop === false) return "wsl.onboarding.updateQueryAI"
}
...
export function addServerViewModel(input: {
  ...
  const queryaiCheck = selectedDistro ? (state?.queryaiChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.queryaiChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateQueryAI" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.queryaiMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installQueryAI" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  queryaiCheck: WslQueryAICheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingQueryAI =
    input.state?.job?.kind === "install-queryai" && input.state.job.distro === input.selectedDistro
  if (!ready || probingQueryAI) {
    return {
      variant: "contrast",
  ...
  if (!addServerQueryAIReady(input.queryaiCheck)) {
    const update = !!input.queryaiCheck?.resolvedPath && input.queryaiCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingQueryAI
        ? { key: "wsl.onboarding.updatingQueryAI" }
        : update
          ? { key: "wsl.onboarding.updateQueryAI" }
          : { key: "wsl.onboarding.installQueryAI" },
      disabled: !!input.state?.job || input.adding,
      action: "install-queryai",
      loading: installingQueryAI,
...
function addServerQueryAIReady(check: WslQueryAICheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.queryaiChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.queryaiChecks[item.name]) return [`queryai:${item.name}`]
    return []
  })
