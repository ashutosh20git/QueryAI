...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when QueryAI needs attention", () => {
    expect(wslQueryAIAction(undefined)).toBeUndefined()
    expect(
      wslQueryAIAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("wsl.onboarding.installQueryAI")
    expect(
      wslQueryAIAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/queryai",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("wsl.onboarding.updateQueryAI")
    expect(
      wslQueryAIAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/queryai",
  ...
  test("does not report ready when QueryAI is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        queryaiChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.queryai/bin/queryai",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
            error: "queryai is installed but could not run",
          },
        },
      },
      view: "main",
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installQueryAI" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-queryai")
  })
