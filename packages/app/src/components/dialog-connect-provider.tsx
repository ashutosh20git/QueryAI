...
import { List, type ListRef } from "@queryai/ui/list"
import { ProviderIcon } from "@queryai/ui/provider-icon"
import { Spinner } from "@queryai/ui/spinner"
import { Tag } from "@queryai/ui/tag"
import { TextField } from "@queryai/ui/text-field"
import { ButtonV2 } from "@queryai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@queryai/ui/v2/dialog-v2"
import { TextInputV2 } from "@queryai/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "queryai-go") return language.t("dialog.provider.queryaiGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "queryai"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.queryai.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["queryai", "queryai-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "queryai" || provider.id === "queryai-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "queryai"
                                ? "dialog.provider.queryai.tagline"
                                : "dialog.provider.queryaiGo.tagline",
                            )}
                          </span>
