import type { Config, QueryAIClient, Path, Project, ProviderAuthResponse, SessionStatus } from "@queryai/sdk/v2/client"
...
import { NormalizedProviderListResponse } from "@queryai/session-ui/context"
...
export const loadLspQuery = (scope: ServerScope, directory: string, sdk: QueryAIClient) =>
  queryOptions({
...
function makeQueryOptionsApi(
  scope: ServerScope,
  serverSDK: () => QueryAIClient,
  serverAPI: ServerApi,
  sdkFor: (dir: PathKey) => QueryAIClient,
  protocol: Promise<"v1" | "v2">,
) {
  return {
...
export function createServerSyncContextInner(serverSDK: ServerSDK) {
  ...
  const sdkCache = new Map<string, QueryAIClient>()
