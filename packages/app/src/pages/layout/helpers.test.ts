...
import { type Session } from "@queryai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  compareSessionTime,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("queryai://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("queryai://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("queryai://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("queryai://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("queryai://open-project")).toBeUndefined()
    expect(parseDeepLink("queryai://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "queryai://open-project?directory=/a",
      "queryai://other?directory=/b",
      "queryai://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("queryai://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("queryai://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("queryai://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("queryai://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "queryai://new-session?directory=/a",
      "queryai://open-project?directory=/b",
      "queryai://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __QUERYAI__: {
        deepLinks: ["queryai://open-project?directory=/a"],
      },
    } as unknown as Window & { __QUERYAI__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["queryai://open-project?directory=/a"])
