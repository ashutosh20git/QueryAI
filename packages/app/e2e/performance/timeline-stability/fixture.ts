...
import { SessionStatusEvent } from "@queryai/schema/session-status-event"
import { SessionV1 } from "@queryai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/QueryAI/TimelineStability"
...
export const model = { providerID: "queryai", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("queryai.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["queryai"],
    default: { providerID: "queryai", modelID: "claude-opus-4-6" },
  }
}
