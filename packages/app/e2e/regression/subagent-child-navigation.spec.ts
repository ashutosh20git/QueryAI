...
import { currentSession, mockQueryAIServer } from "../utils/mock-server"
import { expectSessionTitle } from "../utils/waits"

const directory = "C:/QueryAI/SubagentNavigation"
...
test("navigates to a subagent child session missing from the session list", async ({ page }) => {
  ...
  const titlebarRight = page.locator("#queryai-titlebar-right")
  ...
  return [
    {
      info: {
        ...
        agent: "build",
        model: { providerID: "queryai", modelID: "claude-opus-4-6" },
      },
      parts: [
        {
  ...
  await page.addInitScript(
    ({ directory, server, sessionId }) => {
      ...
      localStorage.setItem(
        "queryai.global.dat:server",
        JSON.stringify({
          projects: { local: [{ worktree: directory, expanded: true }] },
          lastProject: { local: directory },
        }),
      )
      localStorage.setItem("queryai.window.browser.dat:tabs", JSON.stringify([{ type: "session", server, sessionId }]))
    },
