...
import { createQueryAI, type ToolPart } from "@queryai/sdk"
...
console.log("🚀 Starting queryai server...")
const queryai = await createQueryAI({
  port: 0,
})
console.log("✅ QueryAI server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await queryai.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new queryai session...")
    const { client, server } = queryai
    ...
    console.log("✅ Created queryai session:", createResult.data.id)
  ...
  console.log("📝 Sending to queryai:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 QueryAI response:", JSON.stringify(result, null, 2))
