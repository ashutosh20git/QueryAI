...
import { PromptInput } from "@queryai/schema/prompt-input"
import { Session } from "@queryai/schema/session"
import { Project } from "@queryai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@queryai/schema/schema"
...
import { SessionEvent } from "@queryai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this QueryAI process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this QueryAI process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
