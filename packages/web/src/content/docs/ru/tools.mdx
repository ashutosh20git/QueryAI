...
Инструменты позволяют LLM выполнять действия в вашей кодовой базе. queryai поставляется с набором встроенных инструментов, но вы можете расширить его с помощью [пользовательских инструментов](/docs/custom-tools) или [MCP-серверов](/docs/mcp-servers).
...
Вот все встроенные инструменты, доступные в queryai.
...
:::note
Этот инструмент доступен только при `QUERYAI_EXPERIMENTAL_LSP_TOOL=true` (или `QUERYAI_EXPERIMENTAL=true`).
:::
...
:::note
Этот инструмент доступен только при использовании поставщика queryai или когда для переменной среды `QUERYAI_ENABLE_EXA` установлено любое истинное значение (например, `true` или `1`).

Чтобы включить при запуске queryai:
