---
title: LSP-серверы
description: queryai интегрируется с вашими серверами LSP.
---

queryai может интегрироваться с серверами Language Server Protocol (LSP), чтобы использовать диагностику как обратную связь для агента.

---

## Встроенные

queryai поставляется с несколькими встроенными LSP-серверами для популярных языков:
...
:::note
Вы можете отключить автоматическую загрузку LSP-сервера, установив для переменной среды `QUERYAI_DISABLE_LSP_DOWNLOAD` значение `true`.
:::
...
Когда LSP включен и queryai открывает файл, он:
...
Вы можете включить и настроить серверы LSP через раздел `lsp` в конфигурации queryai.
