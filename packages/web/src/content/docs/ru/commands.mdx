...
Создайте `.queryai/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию queryai или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем queryai [config](/docs/config):
...
- Глобальный: `~/.config/queryai/commands/`
- Для каждого проекта: `.queryai/commands/`
...
queryai включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
