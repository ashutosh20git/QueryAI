...
Puoi aggiungere strumenti esterni a QueryAI usando il _Model Context Protocol_, o MCP. QueryAI supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [QueryAI Config](https://opencode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/queryai`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
QueryAI gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, QueryAI:
...
Se il server richiede autenticazione, QueryAI ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `queryai mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, QueryAI memorizzera' i token in modo sicuro in `~/.local/share/queryai/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in QueryAI insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione QueryAI come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare QueryAI al tuo account Sentry.
