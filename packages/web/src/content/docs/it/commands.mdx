...
Puoi aggiungere comandi personalizzati tramite la config di QueryAI o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di QueryAI:
...
- Globale: `~/.config/queryai/commands/`
- Per progetto: `.queryai/commands/`
...
queryai include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).
