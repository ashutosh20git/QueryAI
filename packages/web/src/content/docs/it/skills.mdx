...
Le skill degli agenti permettono a QueryAI di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
QueryAI cerca in queste posizioni:

- Config di progetto: `.queryai/skills/<name>/SKILL.md`
- Config globale: `~/.config/queryai/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, QueryAI risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.queryai/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/queryai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.queryai/skills/git-release/SKILL.md` cosi':
...
QueryAI elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `queryai.json`:
...
**Per agenti integrati** (in `queryai.json`):
