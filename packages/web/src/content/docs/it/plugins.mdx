---
title: Plugin
description: Scrivi plugin per estendere QueryAI.
---

I plugin ti permettono di estendere QueryAI agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di QueryAI.
...
- `.queryai/plugins/` - plugin a livello progetto
- `~/.config/queryai/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/queryai/node_modules/`.
...
1. Config globale (`~/.config/queryai/queryai.json`)
2. Config di progetto (`queryai.json`)
3. Directory plugin globale (`~/.config/queryai/plugins/`)
4. Directory plugin di progetto (`.queryai/plugins/`)
...
QueryAI esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di queryai per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere queryai.
...
:::note
Se usi l'app desktop di QueryAI, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a queryai di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a queryai:
...
L'helper `tool` crea uno strumento personalizzato che queryai può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in queryai insieme agli strumenti integrati.
