...
Con QueryAI puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, QueryAI usa il tema `queryai`.
...
QueryAI include diversi temi integrati.
...
- Vuole che QueryAI corrisponda all'aspetto del terminale
...
QueryAI supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/queryai/themes/*.json` o `$XDG_CONFIG_HOME/queryai/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.queryai/themes/*.json`
4. **Directory di lavoro corrente** - in `./.queryai/themes/*.json`
