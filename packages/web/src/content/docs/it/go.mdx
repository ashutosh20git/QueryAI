...
QueryAI Go è un abbonamento a basso costo da **10 $/mese** che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in QueryAI. Ti abboni a QueryAI Go e
...
QueryAI Go ti dà accesso a questi modelli a **10 $/mese**.

---

## Come funziona

QueryAI Go funziona come qualsiasi altro provider in QueryAI.

1. Accedi a **<a href={console}>QueryAI Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `QueryAI Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a QueryAI Go.
:::
...
QueryAI Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua QueryAI config
utilizza il formato `queryai-go/<model-id>`. Ad esempio, per Kimi K3, useresti
`queryai-go/kimi-k3` nella tua configurazione.
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con QueryAI.
