---
title: Serveur
description: Interagissez avec le serveur queryai via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `queryai serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client queryai peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `queryai.local` |
...
Définissez `QUERYAI_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `queryai`, ou définissez `QUERYAI_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `queryai serve` et à `queryai web`.
...
Lorsque vous exécutez `queryai`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur queryai pour interagir avec queryai par programmation.
:::

Cette architecture permet à queryai de prendre en charge plusieurs clients et vous permet d'interagir avec queryai par programme.

Vous pouvez exécuter `queryai serve` pour démarrer un serveur autonome. Si vous avez le
queryai TUI en cours d'exécution, `queryai serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins QueryAI [IDE](/docs/ide).
...
Le serveur queryai expose les API suivantes.
