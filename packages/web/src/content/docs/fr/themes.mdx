...
Avec QueryAI, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, QueryAI utilise notre propre thème `queryai`.
...
QueryAI est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que QueryAI corresponde à l'apparence de leur terminal
...
QueryAI prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/queryai/themes/*.json` ou `$XDG_CONFIG_HOME/queryai/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.queryai/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.queryai/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/queryai/themes
vim ~/.config/queryai/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .queryai/themes
vim .queryai/themes/my-theme.json
```
