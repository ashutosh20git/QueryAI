---
title: Plugins
description: Écrivez vos propres plugins pour étendre QueryAI.
---

Les plugins vous permettent d'étendre QueryAI en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de QueryAI.
...
- `.queryai/plugins/` - Plugins au niveau du projet
- `~/.config/queryai/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/queryai/node_modules/`.
...
1. Configuration globale (`~/.config/queryai/queryai.json`)
2. Configuration du projet (`queryai.json`)
3. Répertoire global des plugins (`~/.config/queryai/plugins/`)
4. Répertoire des plugins du projet (`.queryai/plugins/`)
...
QueryAI exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK queryai pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre queryai.
...
:::note
Si vous utilisez l'application de bureau QueryAI, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez queryai de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à queryai :
...
L'assistant `tool` crée un outil personnalisé que queryai peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour queryai aux côtés des outils intégrés.
