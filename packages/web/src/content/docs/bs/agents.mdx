...
Postoje dvije vrste agenata u QueryAI: primarni agenti i podagenti.
...
QueryAI dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
QueryAI dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

QueryAI dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u QueryAI-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `queryai.json`:
...
- Globalno: `~/.config/queryai/agents/`
- Po projektu: `.queryai/agents/`
...
Ako temperatura nije navedena, QueryAI koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu QueryAI konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj QueryAI konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [QueryAI Zen](/docs/zen), koristili biste `queryai/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `queryai models` da vidite listu dostupnih modela.
:::
