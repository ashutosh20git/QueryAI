---
title: Dodaci
description: Napišite vlastite dodatke za proširenje QueryAI.
---

Dodaci vam omogućavaju da proširite QueryAI spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja QueryAI.
...
- `.queryai/plugins/` - Dodaci na nivou projekta
- `~/.config/queryai/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/queryai/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/queryai/queryai.json`)
2. Konfiguracija projekta (`queryai.json`)
3. Globalni direktorij dodataka (`~/.config/queryai/plugins/`)
4. Direktorij dodataka projekta (`.queryai/plugins/`)
...
QueryAI pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: QueryAI SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje QueryAI.
...
Spriječite queryai da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u queryai:
...
Pomoćnik `tool` kreira prilagođeni alat koji queryai može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za queryai zajedno sa ugrađenim alatima.
