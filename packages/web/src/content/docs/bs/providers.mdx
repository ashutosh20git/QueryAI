---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u QueryAI.
---

import config from "../../../../config.mjs"
export const console = config.console

QueryAI koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj QueryAI konfiguraciji.
...
u `~/.local/share/queryai/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem QueryAI
config.
...
QueryAI Zen je lista modela koje je obezbedio QueryAI tim koji su bili
testirano i potvrđeno da dobro radi sa QueryAI. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa QueryAI Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite queryai i idite na [opencode.ai/auth](https://opencode.ai/auth).
...
Radi kao i svaki drugi provajder u QueryAI i potpuno je opcionalan za korištenje.

---

## QueryAI Go

QueryAI Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža QueryAI tim i koji su testirani i verificirani da dobro rade s QueryAI-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `QueryAI Go` i idite na [opencode.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s QueryAI:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete queryai:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `queryai.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u QueryAI.
:::
...
Možete konfigurirati queryai za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi queryai ispravno radio.
   :::
...
Također možete dodati modele kroz svoju queryai konfiguraciju.
...
da pohrani token u queryai auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
QueryAI koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate QueryAI
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`queryai.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `queryai.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s queryai:
...
Za korištenje Google Vertex AI s QueryAI:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće queryai.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem queryaia, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki QueryAI razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki QueryAI razgovor naveden kao zasebna sesija.
...
Možete konfigurirati queryai za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati queryai za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati queryai za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za QueryAI. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s QueryAI:
...
3. Kopirajte API ključ za korištenje u QueryAI.
...
6. **Važno**: Prije upotrebe modela oblaka u QueryAI, morate lokalno povući informacije o modelu:
...
QueryAI Zen je lista testiranih i verifikovanih modela koju je obezbedio QueryAI tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>QueryAI Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **QueryAI Zen**.

```txt
   /connect
```

3. Unesite svoj QueryAI API ključ.
...
Također možete dodati dodatne modele putem vaše queryai konfiguracije.
...
5. Također ih možete prilagoditi putem vaše queryai konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa QueryAIom:
...
Također možete prilagoditi modele kroz svoju queryai konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s queryai-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `queryai.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju QueryAI da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `queryai auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite QueryAI konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj queryai konfiguraciji.
