---
title: Konfiguracija
description: Korištenje QueryAI JSON konfiguracije.
---

Možete konfigurirati QueryAI koristeći JSON konfiguracijski fajl.

---

## Format

QueryAI podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/queryai`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/queryai/queryai.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`QUERYAI_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`queryai.json` u projektu) - postavke specifične za projekat
5. **`.queryai` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`QUERYAI_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.queryai` i `~/.config/queryai` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/queryai` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu QueryAI konfiguraciju u `~/.config/queryai/queryai.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/queryai/tui.json`.
...
Dodajte `queryai.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se QueryAI pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `QUERYAI_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `QUERYAI_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.queryai` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.queryai`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`opencode.ai/config.json`**](https://raw.githubusercontent.com/ashutosh20git/QueryAI-dist/schema/config.json).
...
Koristite `QUERYAI_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `queryai.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `queryai serve` i `queryai web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš QueryAI server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `queryai.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj QueryAI konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, QueryAI pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/queryai/agents/` ili `.queryai/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, QueryAI će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`queryai run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/queryai/commands/` ili `.queryai/commands/`. [Saznajte više ovdje](/docs/commands).
...
QueryAI će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, QueryAI **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju QueryAI sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.queryai/plugins/` ili `~/.config/queryai/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite QueryAI da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
