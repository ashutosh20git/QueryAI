...
Da biste otklonili probleme s QueryAI, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\queryai\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `queryai --log-level DEBUG`.

---

## Pohrana

QueryAI pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/queryai/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\queryai`
...
QueryAI Desktop pokreće lokalni QueryAI server (`queryai-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `QueryAI` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/queryai/queryai.jsonc` (ili `~/.config/queryai/queryai.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/queryai/queryai.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\queryai\queryai.jsonc`
...
QueryAI također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/queryai/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\queryai\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga QueryAI mogao ponovo izgraditi.

1. Potpuno zatvorite QueryAI Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/queryai`
- **Linux**: obrišite `~/.cache/queryai` (ili pokrenite `rm -rf ~/.cache/queryai`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\queryai`

3. Ponovo pokrenite QueryAI Desktop.

---

### Rješavanje problema sa vezom na serveru

QueryAI Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `queryai.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `QUERYAI_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `QUERYAI_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, QueryAI Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa QueryAI karakteristikama.

---

### Obavještenja se ne prikazuju

QueryAI Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za QueryAI u postavkama vašeg OS-a, i
...
1. Zatvorite QueryAI Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka QueryAI Desktop aplikacije):

- `queryai.settings.dat` (URL zadanog servera za desktop)
- `queryai.global.dat` i `queryai.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `queryai models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\queryai`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. QueryAI dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\queryai`

2. Ponovo pokrenite QueryAI da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti QueryAI da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
QueryAI će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
