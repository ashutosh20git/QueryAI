...
Možete dodati vanjske alate u QueryAI koristeći _Model Context Protocol_, ili MCP. QueryAI podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [QueryAI Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/queryai`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
QueryAI automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, QueryAI će:
...
Ako server zahtijeva autentifikaciju, QueryAI će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `queryai mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, QueryAI sigurno čuva tokene u `~/.local/share/queryai/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u QueryAI, zajedno s ugrađenim alatima. Možete njima upravljati kroz QueryAI konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete QueryAI sa vašim Sentry nalogom.
