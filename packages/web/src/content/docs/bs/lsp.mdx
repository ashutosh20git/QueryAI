---
title: LSP serveri
description: QueryAI se integriše sa vašim LSP serverima.
---

QueryAI se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

QueryAI dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `QUERYAI_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i queryai otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj queryai konfiguraciji.
