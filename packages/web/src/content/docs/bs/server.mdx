---
title: Server
description: Komunicirajte s queryai serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `queryai serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji queryai klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `queryai.local` |
...
Postavite `QUERYAI_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `queryai`, ili postavite `QUERYAI_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `queryai serve` i za `queryai web`.
...
Kada pokrenete `queryai`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite queryai server za programsku interakciju sa queryai-om.
:::

Ova arhitektura omogucava queryai podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `queryai serve` da startate standalone server. Ako je queryai TUI vec pokrenut, `queryai serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste QueryAI [IDE](/docs/ide) pluginovi.
...
queryai server izlaže sljedece API-je.
