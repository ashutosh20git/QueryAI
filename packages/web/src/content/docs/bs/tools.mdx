...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. QueryAI dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u QueryAI.
...
:::note
Ovaj alat je dostupan samo kada je `QUERYAI_EXPERIMENTAL_LSP_TOOL=true` (ili `QUERYAI_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz QueryAI provajdera ili kada je varijabla `QUERYAI_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju QueryAI:
