...
QueryAI Go je povoljna pretplata od **$10/mjesečno** koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u QueryAI-u. Pretplatite se na QueryAI Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili QueryAI.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa QueryAI-om.
:::
...
QueryAI Go vam daje pristup ovim modelima za **$10/mjesečno**.

---

## Kako funkcioniše

QueryAI Go radi kao bilo koji drugi provajder u QueryAI-u.

1. Prijavite se na **<a href={console}>QueryAI Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `QueryAI Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na QueryAI Go.
:::
...
QueryAI Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj QueryAI konfiguraciji
koristi format `queryai-go/<model-id>`. Na primjer, za Kimi K3, koristili biste
`queryai-go/kimi-k3` u svojoj konfiguraciji.
...
Napravili smo QueryAI Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz QueryAI koristite i bilo kojeg drugog provajdera.
