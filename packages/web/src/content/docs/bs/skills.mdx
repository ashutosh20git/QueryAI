...
Agent skills omogucavaju QueryAI da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
QueryAI pretrazuje ove lokacije:

- Konfiguracija projekta: `.queryai/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/queryai/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, QueryAI ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.queryai/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/queryai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.queryai/skills/git-release/SKILL.md` ovako:
...
QueryAI navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `queryai.json`:
...
**Za ugradene agente** (u `queryai.json`):
