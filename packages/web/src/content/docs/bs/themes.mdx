...
U QueryAI mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, QueryAI koristi nasu `queryai` temu.
...
QueryAI dolazi sa vise ugradenih tema.
...
- Zele da QueryAI odgovara izgledu njihovog terminala
...
QueryAI podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/queryai/themes/*.json` ili `$XDG_CONFIG_HOME/queryai/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.queryai/themes/*.json`
4. **Trenutni radni direktorij** - `./.queryai/themes/*.json`
