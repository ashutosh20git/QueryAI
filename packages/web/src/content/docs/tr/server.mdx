---
title: Sunucu
description: queryai sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`queryai serve` komutu, queryai istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `queryai.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `QUERYAI_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `queryai` değeridir; değiştirmek isterseniz `QUERYAI_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `queryai serve` hem de `queryai web` için geçerlidir.
...
`queryai` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
queryai ile programatik etkileşim için queryai sunucusunu kullanın.
:::

Bu mimari, queryai'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`queryai serve` ile bağımsız bir sunucu başlatabilirsiniz. queryai TUI açıksa, `queryai serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum queryai [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
queryai sunucusu aşağıdaki API'leri sunar.
