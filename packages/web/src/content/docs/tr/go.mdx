...
QueryAI Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli **aylık 10$ aboneliğidir**.

Go, QueryAI'daki diğer sağlayıcılar gibi çalışır. QueryAI Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve QueryAI'u kullanmak için bunu kullanmanıza gerek yoktur.
...
QueryAI Go, bu modellere **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

QueryAI Go, QueryAI'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>QueryAI Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `QueryAI Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye QueryAI Go'ya abone olabilir.
:::
...
QueryAI Go aşağıdaki limitleri içerir:
...
QueryAI yapılandırmanızdaki [model id](/docs/config/#models) formatı `queryai-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `queryai-go/kimi-k3` kullanmalısınız.
...
QueryAI Go'yu şu amaçlarla oluşturduk:
...
4. QueryAI ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
