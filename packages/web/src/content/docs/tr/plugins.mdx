---
title: Eklentiler
description: QueryAI'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek QueryAI'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya QueryAI'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.queryai/plugins/` - Proje düzeyinde eklentiler
- `~/.config/queryai/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/queryai/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/queryai/queryai.json`)
2. Project config (`queryai.json`)
3. Global eklenti dizini (`~/.config/queryai/plugins/`)
4. Proje eklenti dizini (`.queryai/plugins/`)
...
QueryAI bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir QueryAI SDK istemcisi.
...
QueryAI'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
QueryAI masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
QueryAI'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca QueryAI'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, QueryAI'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra QueryAI'a da hazır olacaktır.
