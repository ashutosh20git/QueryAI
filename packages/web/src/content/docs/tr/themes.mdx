...
queryai ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak queryai kendi `queryai` temasını kullanır.
...
queryai birden fazla yerleşik temayla gelir.
...
- queryai'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
queryai, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/queryai/themes/*.json` veya `$XDG_CONFIG_HOME/queryai/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.queryai/themes/*.json`
4. **Geçerli çalışma dizini** - `./.queryai/themes/*.json`
