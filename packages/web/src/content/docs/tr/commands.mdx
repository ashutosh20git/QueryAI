...
`.queryai/commands/test.md` oluştur:
...
queryai işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

queryai [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/queryai/commands/`
- Proje başına: `.queryai/commands/`
...
queryai, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
