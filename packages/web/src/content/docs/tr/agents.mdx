...
queryai'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
queryai, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
QueryAI, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

QueryAI iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `queryai.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse queryai modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global queryai yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
queryai yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [QueryAI Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `queryai/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `queryai models` komutunu çalıştırın.
:::
