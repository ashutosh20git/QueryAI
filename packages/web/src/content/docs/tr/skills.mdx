...
Ajan becerileri, queryai'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
queryai su konumlari tarar:

- Proje konfigurasyonu: `.queryai/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/queryai/skills/<name>/SKILL.md`
...
Proje yerel yollarinda queryai, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.queryai/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/queryai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.queryai/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
queryai, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`queryai.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`queryai.json` icinde):
