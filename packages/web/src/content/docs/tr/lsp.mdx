...
description: QueryAI, LSP sunucularınızla bütünleşir.
---

QueryAI, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

QueryAI, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`QUERYAI_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve queryai bir dosyayı açtığında:
...
LSP sunucularını, queryai yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
