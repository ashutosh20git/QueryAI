...
description: queryai dile özgü biçimlendiriciler kullanır.
---

queryai, dosyaları dile özgü formatlayıcılar kullanılarak yazıldıktan veya düzenlendikten sonra otomatik olarak formatlar. Bu, oluşturulan kodun projenizin kod stillerine uymasını sağlar.

---

## Yerleşik

queryai, popüler diller ve çerçeveler için çeşitli yerleşik biçimlendiricilerle birlikte gelir. Aşağıda ihtiyaç duyduğu biçimlendiricilerin, desteklenen dosya uzantılarının ve komutların veya yapılandırma seçeneklerinin bir listesi bulunmaktadır.
...
Yani eğer projenizin `package.json` dosyasında `prettier` varsa, queryai bunu otomatik olarak kullanacaktır.

---

## Nasıl çalışır?

queryai bir dosyayı yazdığında veya düzenlediğinde:
...
Biçimlendiricileri queryai yapılandırmanızdaki `formatter` bölümü aracılığıyla özelleştirebilirsiniz.
