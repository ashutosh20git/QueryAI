...
_Model Bağlam Protokolü_ veya MCP kullanarak queryai'a harici araçlar ekleyebilirsiniz. queryai hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [queryai Config](https://opencode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/queryai` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
queryai, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde queryai şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa queryai, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `queryai mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından queryai, belirteçleri `~/.local/share/queryai/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra queryai'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi queryai yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve queryai'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
