...
أنشئ الملف `.queryai/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات QueryAI أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ QueryAI:
...
- على مستوى النظام: `~/.config/queryai/commands/`
- لكل مشروع: `.queryai/commands/`
...
يتضمن queryai عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
