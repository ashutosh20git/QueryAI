...
يمكنك إضافة أدوات خارجية إلى QueryAI باستخدام _Model Context Protocol_، أو MCP. يدعم QueryAI خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات QueryAI](https://opencode.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/queryai` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى QueryAI تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم QueryAI بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك QueryAI المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `queryai mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن QueryAI الرموز بشكل آمن في `~/.local/share/queryai/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل QueryAI إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات QueryAI مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط QueryAI بحساب Sentry الخاص بك.
