...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في queryai.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في queryai مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.queryai/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/queryai/tools/`.
