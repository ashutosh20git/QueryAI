...
تتيح مهارات الوكيل لـ QueryAI اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث QueryAI في هذه المواقع:

- إعدادات المشروع: `.queryai/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/queryai/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج QueryAI صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.queryai/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/queryai/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.queryai/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض QueryAI المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `queryai.json`:
...
**للوكلاء المدمجين** (في `queryai.json`):
