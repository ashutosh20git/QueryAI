---
title: Zen
description: قائمة منتقاة من النماذج يوفّرها QueryAI.
---
...
QueryAI Zen هي قائمة بالنماذج التي اختبرها فريق QueryAI وتحقّق منها.

يعمل Zen مثل أي مزوّد آخر في QueryAI. تسجّل الدخول إلى QueryAI Zen وتحصل على
مفتاح API. استخدامه **اختياري بالكامل** ولا تحتاج إليه لاستخدام QueryAI.
...
:::tip
اختبرنا مجموعة منتقاة من النماذج والمزوّدين الذين يعملون جيدا مع QueryAI.
:::
...
QueryAI Zen هي بوابة AI تتيح لك الوصول إلى هذه النماذج.

---

## كيف يعمل

يعمل QueryAI Zen مثل أي مزوّد آخر في QueryAI.

1. تسجّل الدخول إلى **<a href={console}>QueryAI Zen</a>**، وتضيف تفاصيل الفوترة، ثم تنسخ مفتاح API.
2. تشغّل الأمر `/connect` في واجهة TUI، وتختار QueryAI Zen، ثم تلصق مفتاح API.
...
يستخدم [معرّف النموذج](/docs/config/#models) في إعدادات QueryAI الصيغة `queryai/<model-id>`. على سبيل المثال، بالنسبة إلى GPT 5.5، ستستخدم `queryai/gpt-5.5` في إعداداتك.
...
قد تلاحظ [نماذج منخفضة التكلفة](/docs/config/#models)، مثل Haiku أو Nano أو Flash، في سجل الاستخدام. يستخدم QueryAI هذه النماذج لإنشاء عناوين الجلسات.
...
- MiMo-V2.5 Free متاح على QueryAI لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Ling 3.0 Flash Fin Free متاح على QueryAI لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Nemotron 3 Ultra Free متاح على QueryAI لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Nemotron 3.5 Lightning Free متاح على QueryAI لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Big Pickle نموذج خفي ومتاح مجانا على QueryAI لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Muse Spark 1.2 Contributor Free متاح على QueryAI لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
...
4. **عدم فرض أي ارتباط حصري** عبر تمكينك من استخدامه مع أي عامل برمجة آخر. ومع إبقاء إمكانية استخدام أي مزوّد آخر مع QueryAI أيضا دائما.
