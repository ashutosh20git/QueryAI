---
title: المزوّدون
description: استخدام أي موفّر LLM في QueryAI.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم QueryAI ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات QueryAI.
...
في `~/.local/share/queryai/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات QueryAI.
...
QueryAI Zen هي قائمة نماذج يوفّرها فريق QueryAI وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع QueryAI. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع QueryAI Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر queryai، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/auth).
...
يعمل مثل أي مزوّد آخر في QueryAI واستخدامه اختياري بالكامل.

---

## QueryAI Go

QueryAI Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق QueryAI، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع QueryAI.

1. شغّل الأمر `/connect` في TUI، واختر `QueryAI Go`، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/zen).
...
لاستخدام Amazon Bedrock مع QueryAI:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل queryai:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `queryai.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل QueryAI غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين queryai لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل queryai بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات queryai.
...
تخزين الرمز في مساحة تخزين مصادقة queryai.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم QueryAI نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر QueryAI
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`queryai.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع queryai:
...
لاستخدام Google Vertex AI مع QueryAI:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر queryai، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة QueryAI تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة QueryAI مدرجة كجلسة مستقلة.
...
يمكنك ضبط queryai لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط queryai لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط queryai لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع QueryAI. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع QueryAI:
...
3. انسخ مفتاح API لاستخدامه في QueryAI.
...
6. **مهم**: قبل استخدام نماذج السحابة في QueryAI، يجب سحب معلومات النموذج محليا:
...
QueryAI Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق QueryAI. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>QueryAI Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **QueryAI Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ QueryAI.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات queryai.
...
5. يمكنك أيضا تخصيصها عبر إعدادات queryai. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع QueryAI:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات queryai. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع queryai. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `queryai.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ QueryAI بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `queryai auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات queryai وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات queryai.
