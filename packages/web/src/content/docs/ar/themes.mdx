...
مع QueryAI يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم QueryAI سمتنا `queryai`.
...
يأتي QueryAI مع عدة سمات مدمجة.
...
- يريدون أن يطابق QueryAI مظهر terminal لديهم
...
يدعم QueryAI نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/queryai/themes/*.json` أو `$XDG_CONFIG_HOME/queryai/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.queryai/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.queryai/themes/*.json`
