...
QueryAI Goは、人気のオープンなコーディングモデルに安定してアクセスできる、低価格の**月額10ドルのサブスクリプション**です。

GoはQueryAIの他のプロバイダーと同様に機能します。QueryAI GoをサブスクライブしてAPIキーを取得します。これは**完全に任意**であり、QueryAIを使用するために必須ではありません。
...
:::tip
QueryAIでうまく動作する一部のモデルとプロバイダーをテストしました。
:::
...
QueryAI Goを使用すると、これらのモデルに**月額10ドル**でアクセスできます。

---

## 仕組み

QueryAI Goは、QueryAIの他のプロバイダーと同様に機能します。

1. **<a href={console}>QueryAI Zen</a>**にサインインし、GoをサブスクライブしてAPIキーをコピーします。
2. TUIで`/connect`コマンドを実行し、`QueryAI Go`を選択して、APIキーを貼り付けます。
3. TUIで`/models`を実行すると、Goを通じて利用可能なモデルのリストが表示されます。

:::note
QueryAI Goをサブスクライブできるのは、1つのワークスペースにつき1メンバーのみです。
:::
...
QueryAI設定の[model id](/docs/config/#models)は、`queryai-go/<model-id>`という形式を使用します。たとえば、Kimi K3の場合は、設定で`queryai-go/kimi-k3`を使用します。
...
4. QueryAIで他のプロバイダーも使用できるようにすることで、**ベンダーロックインをなくす**。
