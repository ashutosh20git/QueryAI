---
title: QueryAI Zen
description: QueryAI によって提供されるモデルの厳選されたリスト。
---
...
QueryAI Zen は、QueryAI チームが提供する、テスト済みかつ検証済みのモデルの一覧です。

Zen は QueryAI のほかのプロバイダーと同じように動作します。QueryAI Zen にログインして API キーを取得できます。これは **完全にオプション** であり、QueryAI を使うために必須ではありません。
...
:::tip
QueryAI でうまく動作する、厳選したモデルとプロバイダーの組み合わせをテストしました。
:::
...
QueryAI Zen は、これらのモデルにアクセスできる AI ゲートウェイです。

---

## 仕組み

QueryAI Zen は、QueryAI のほかのプロバイダーと同じように動作します。

1. **<a href={console}>QueryAI Zen</a>** にサインインし、請求情報を追加して、API キーをコピーします。
2. TUI で `/connect` コマンドを実行し、QueryAI Zen を選んで API キーを貼り付けます。
...
QueryAI 設定で使う [model id](/docs/config/#models) は `queryai/<model-id>` 形式です。たとえば、GPT 5.5 では設定に `queryai/gpt-5.5` を使用します。
...
使用履歴に Haiku、Nano、Flash などの[低コストモデル](/docs/config/#models)が表示されることがあります。QueryAI はこれらのモデルをセッションタイトルの生成に使用します。
...
- MiMo-V2.5 Free は期間限定で QueryAI で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Ling 3.0 Flash Fin Free は期間限定で QueryAI で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Nemotron 3 Ultra Free は期間限定で QueryAI で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Nemotron 3.5 Lightning Free は期間限定で QueryAI で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Big Pickle はステルスモデルで、期間限定で QueryAI で無料提供されています。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Muse Spark 1.2 Contributor Free は期間限定で QueryAI で利用できます。チームはこの期間を活用してフィードバックを収集し、モデルを改善しています。
...
私たちが QueryAI Zen を作った目的は次のとおりです。
...
4. ほかのコーディングエージェントでも使えるようにして **no lock-in** を実現し、さらに QueryAI では常にほかのプロバイダーも使えるようにすること。
