---
title: サーバー
description: HTTP 経由で QueryAI サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`queryai serve` コマンドは、QueryAI クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `queryai.local` |
...
HTTP 基本認証でサーバーを保護するには、`QUERYAI_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `queryai` になるか、`QUERYAI_SERVER_USERNAME` を設定してオーバーライドします。これは、`queryai serve` と `queryai web` の両方に当てはまります。
...
`queryai` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
QueryAI サーバーを使用して、プログラムで QueryAI と対話します。
:::

`queryai serve` を実行してスタンドアロンサーバーを起動できます。すでに QueryAI TUI を実行している場合でも、`queryai serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、QueryAI [IDE](/docs/ide) プラグインによって使用されます。
