...
With QueryAI you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, QueryAI uses our own `queryai` theme.
...
QueryAI comes with several built-in themes.
...
- Want QueryAI to match their terminal's appearance
...
QueryAI supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/queryai/themes/*.json` or `$XDG_CONFIG_HOME/queryai/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.queryai/themes/*.json`
4. **Current working directory** - Defined in `./.queryai/themes/*.json`
