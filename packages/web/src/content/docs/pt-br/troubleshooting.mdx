...
Para depurar problemas com o queryai, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\queryai\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `queryai --log-level DEBUG`.

---

## Armazenamento

O queryai armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/queryai/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\queryai`
...
O queryai Desktop executa um servidor queryai local (o sidecar `queryai-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `QueryAI` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/queryai/queryai.jsonc` (ou `~/.config/queryai/queryai.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/queryai/queryai.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\queryai\queryai.jsonc`
...
O queryai também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/queryai/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\queryai\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o queryai possa reconstruí-lo.

1. Saia completamente do queryai Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/queryai`
- **Linux**: exclua `~/.cache/queryai` (ou execute `rm -rf ~/.cache/queryai`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\queryai`

3. Reinicie o queryai Desktop.

---

### Corrigindo problemas de conexão com o servidor

O queryai Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `queryai.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `QUERYAI_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `QUERYAI_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o queryai Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do queryai.

---

### Notificações não aparecendo

O queryai Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o queryai nas configurações do seu sistema operacional, e
...
1. Saia do queryai Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo queryai Desktop):

- `queryai.settings.dat` (URL do servidor padrão do desktop)
- `queryai.global.dat` e `queryai.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o queryai:
...
Para descobrir quais modelos você tem acesso, execute `queryai models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\queryai`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O queryai instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\queryai`

2. Reinicie o queryai para reinstalar os pacotes de provedores mais recentes

Isso forçará o queryai a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O queryai detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
