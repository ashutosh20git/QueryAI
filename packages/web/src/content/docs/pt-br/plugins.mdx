---
title: Plugins
description: Escreva seus próprios plugins para estender o queryai.
---

Plugins permitem que você estenda o queryai conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do queryai.
...
- `.queryai/plugins/` - Plugins em nível de projeto
- `~/.config/queryai/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/queryai/node_modules/`.
...
1. Configuração global (`~/.config/queryai/queryai.json`)
2. Configuração do projeto (`queryai.json`)
3. Diretório de plugins global (`~/.config/queryai/plugins/`)
4. Diretório de plugins do projeto (`.queryai/plugins/`)
...
O queryai executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do queryai para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o queryai.
...
:::note
Se você estiver usando o aplicativo desktop queryai, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o queryai de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao queryai:
...
O helper `tool` cria uma ferramenta personalizada que o queryai pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o queryai junto com as ferramentas integradas.
