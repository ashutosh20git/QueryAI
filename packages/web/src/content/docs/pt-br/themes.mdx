...
Com o queryai, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o queryai usa nosso próprio tema `queryai`.
...
O queryai vem com vários temas embutidos.
...
- Querem que o queryai corresponda à aparência do seu terminal
...
O queryai suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/queryai/themes/*.json` ou `$XDG_CONFIG_HOME/queryai/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.queryai/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.queryai/themes/*.json`
