...
Existem dois tipos de agentes no queryai; agentes primários e subagentes.
...
queryai vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
QueryAI vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

QueryAI vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do QueryAI, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `queryai.json`:
...
- Global: `~/.config/queryai/agents/`
- Por projeto: `.queryai/agents/`

```markdown title="~/.config/queryai/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o queryai usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do queryai quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do queryai usa o formato `provider/model-id`. Por exemplo, se você estiver usando [QueryAI Zen](/docs/zen), você usaria `queryai/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="queryai.json" {3-6,9-12}
{
  "$schema": "https://raw.githubusercontent.com/ashutosh20git/QueryAI-dist/schema/config.json",
  "tools": {
    "write": true,
...
```json title="queryai.json" {3-5,8-10}
{
  "$schema": "https://raw.githubusercontent.com/ashutosh20git/QueryAI-dist/schema/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `queryai models` para ver uma lista dos modelos disponíveis.
:::
...
```markdown title="~/.config/queryai/agents/docs-writer.md"
---
...
```markdown title="~/.config/queryai/agents/security-auditor.md"
---
