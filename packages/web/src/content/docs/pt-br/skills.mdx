...
As habilidades do agente permitem que o queryai descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O queryai pesquisa nesses locais:

- Configuração do projeto: `.queryai/skills/<name>/SKILL.md`
- Configuração global: `~/.config/queryai/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o queryai sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.queryai/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/queryai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.queryai/skills/git-release/SKILL.md` assim:
...
O queryai lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `queryai.json`:
...
**Para agentes embutidos** (em `queryai.json`):
