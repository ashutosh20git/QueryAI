---
title: Servidor
description: Interaja com o servidor queryai via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `queryai serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente queryai pode usar.
...
```bash
queryai serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `queryai.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`            |

`--cors` pode ser passado várias vezes:

```bash
queryai serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `QUERYAI_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `queryai`, ou defina `QUERYAI_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `queryai serve` quanto ao `queryai web`.

```bash
QUERYAI_SERVER_PASSWORD=your-password queryai serve
```
...
Quando você executa `queryai`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor queryai para interagir com o queryai programaticamente.
:::

Essa arquitetura permite que o queryai suporte múltiplos clientes e permite que você interaja com o queryai programaticamente.

Você pode executar `queryai serve` para iniciar um servidor autônomo. Se você tiver o TUI do queryai em execução, `queryai serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do queryai [IDE](/docs/ide).
...
O servidor queryai expõe as seguintes APIs.
