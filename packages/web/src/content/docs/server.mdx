---
title: Server
description: Interact with queryai server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `queryai serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an queryai client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `queryai.local` |
...
Set `QUERYAI_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `queryai`, or set `QUERYAI_SERVER_USERNAME` to override it. This applies to both `queryai serve` and `queryai web`.
...
When you run `queryai` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the queryai server to interact with queryai programmatically.
:::

This architecture lets queryai support multiple clients and allows you to interact with queryai programmatically.

You can run `queryai serve` to start a standalone server. If you have the
queryai TUI running, `queryai serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the QueryAI [IDE](/docs/ide) plugins.
...
The queryai server exposes the following APIs.
