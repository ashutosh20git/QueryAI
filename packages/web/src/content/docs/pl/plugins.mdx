---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć queryai.
---

Wtyczki umożliwiające wyłączenie queryai poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania queryai.
...
- `.queryai/plugins/` - Wtyczki na poziomie projektu
- `~/.config/queryai/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/queryai/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/queryai/queryai.json`)
2. Konfiguracja projektu (`queryai.json`)
3. Globalny katalog wtyczek (`~/.config/queryai/plugins/`)
4. Katalog wtyczek projektu (`.queryai/plugins/`)
...
queryai uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z queryai do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do queryai.
...
:::note
Jeśli korzystasz z aplikacji komputerowej queryai, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj queryai czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do queryai:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod queryai. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
