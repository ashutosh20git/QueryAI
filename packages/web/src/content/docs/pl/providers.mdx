---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w queryai.
---

import config from "../../../../config.mjs"
export const console = config.console

queryai używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji queryai.
...
w `~/.local/share/queryai/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim queryai
config.
...
QueryAI Zen to lista modeli dostarczonych przez zespół queryai, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z queryai. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od QueryAI Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `QueryAI Zen` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
Działa jak każdy inny dostawca w queryai i jest całkowicie opcjonalny w użyciu.

---

## QueryAI Go

QueryAI Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół queryai, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z queryai.

1. Uruchom polecenie `/connect` w TUI, wybierz `QueryAI Go` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
Aby używać Amazon Bedrock z queryai:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania queryai:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `queryai.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w queryai nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować queryai do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod queryai działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji queryai.
...
queryai używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować queryai
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`queryai.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem queryai:
...
Aby używać Google Vertex AI z queryai:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania queryai.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą queryai, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session), aby automatycznie rejestrować każdą konwersację queryai jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja queryai będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować queryai tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować queryai tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować queryai tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem queryai. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/opencode).
:::
...
Aby korzystać z Ollama Cloud z queryai:
...
3. Skopiuj klucz API do użycia w queryai.
...
6. **Ważne**: Przed użyciem modeli chmurowych w queryai musisz pobrać informacje o modelu lokalnie:
...
QueryAI Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół queryai. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>QueryAI Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **QueryAI Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API queryai.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji queryai.
...
5. Możesz je także dostosować za pomocą konfiguracji queryai. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z queryai:
...
Możesz także dostosować modele za pomocą konfiguracji queryai. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z queryai. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `queryai.json` w katalogu projektu:
...
Pola `limit` pozwalają queryai zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `queryai auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację queryai i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji queryai.
