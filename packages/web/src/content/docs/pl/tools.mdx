...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. QueryAI posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w QueryAI.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `QUERYAI_EXPERIMENTAL_LSP_TOOL=true` (lub `QUERYAI_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy QueryAI lub gdy zmienna środowiskowa `QUERYAI_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu QueryAI:
