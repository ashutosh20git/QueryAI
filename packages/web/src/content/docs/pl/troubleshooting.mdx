...
Aby debugować problemy z queryai, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\queryai\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `queryai --log-level DEBUG`.

---

## Przechowywanie danych

queryai przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/queryai/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\queryai`
...
queryai Desktop uruchamia w tle lokalny serwer queryai (sidecar `queryai-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `QueryAI` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/queryai/queryai.jsonc` (lub `~/.config/queryai/queryai.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/queryai/queryai.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\queryai\queryai.jsonc`
...
queryai może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/queryai/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\queryai\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby queryai mógł ją odbudować.

1. Całkowicie zamknij queryai Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/queryai`
- **Linux**: usuń `~/.cache/queryai` (lub uruchom `rm -rf ~/.cache/queryai`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\queryai`

3. Uruchom ponownie queryai Desktop.

---

### Napraw problemy z połączeniem z serwerem

queryai Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `queryai.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `QUERYAI_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `QUERYAI_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows queryai Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami queryai.

---

### Brak powiadomień

queryai Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla queryai w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie queryai Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji queryai Desktop):

- `queryai.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `queryai.global.dat` i `queryai.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `queryai upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `queryai models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\queryai`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. queryai dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\queryai`

2. Uruchom ponownie kod queryai, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to queryai do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
queryai wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
