...
W QueryAI obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
QueryAI zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
QueryAI ma trzech wbudowanych subagentów: **General**, **Explore** i **Scout**. Przyjrzymy się im poniżej.

---

## Wbudowane

QueryAI ma dwa wbudowane agenty główne i trzech wbudowanych subagentów.
...
Agent tylko do odczytu do pracy z zewnętrzną dokumentacją i badaniem zależności. Używaj go, gdy chcesz sklonować repozytorium zależności do zarządzanej pamięci podręcznej QueryAI, przejrzeć kod źródłowy biblioteki albo porównać lokalny kod z implementacjami upstream bez modyfikowania swojego workspace.
...
Skonfiguruj agenta w pliku `queryai.json`:
...
- Globalnie: `~/.config/queryai/agents/`
- W projekcie: `.queryai/agents/`
...
Jeśli nie zostanie skonfigurowana, QueryAI używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji QueryAI, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji QueryAI w `provider/model-id`. Na przykład, [QueryAI Zen](/docs/zen), użyj `queryai/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `queryai models`, aby zobaczyć dostępne modele.
:::
