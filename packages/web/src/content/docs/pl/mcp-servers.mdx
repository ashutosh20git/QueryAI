...
Możesz dodać dodatkowe informacje do queryai za pomocą _Model Context Protocol_, w skrócie MCP. queryai obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [queryai Config](https://opencode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/queryai`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
queryai automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, queryai:
...
Jeśli serwer wymaga uwierzytelnienia, queryai poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `queryai mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji queryai bezpieczne przechowa tokeny w `~/.local/share/queryai/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w queryai, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie queryai, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć queryai z kontem Sentry.
