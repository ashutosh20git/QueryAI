---
title: Serwery LSP
description: QueryAI integruje się z serwerami LSP.
---

QueryAI może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

QueryAI posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `QUERYAI_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i QueryAI otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji QueryAI.
