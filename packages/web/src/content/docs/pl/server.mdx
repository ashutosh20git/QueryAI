---
title: Serwer
description: Interakcja z serwerem queryai za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `queryai serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient queryai.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `queryai.local` |
...
Ustaw `QUERYAI_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `queryai` lub ustaw `QUERYAI_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `queryai serve`, jak i `queryai web`.
...
Kiedy uruchomisz `queryai`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera queryai do programowej interakcji z queryai.
:::

Ta architektura umożliwia obsługę wielu klientów przez queryai i programową interakcję z kodem otwartym.

Możesz uruchomić `queryai serve`, aby uruchomić samodzielny serwer. Jeśli masz
queryai TUI działa, `queryai serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki queryai [IDE](/docs/ide).
...
Serwer queryai udostępnia następujące interfejsy API.
