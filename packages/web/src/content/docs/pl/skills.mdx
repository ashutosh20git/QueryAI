...
Umiejętności agenta pozwalają queryai odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
queryai przeszukuje te lokalizacje:

- Project config: `.queryai/skills/<name>/SKILL.md`
- Global config: `~/.config/queryai/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu queryai przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.queryai/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/queryai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.queryai/skills/git-release/SKILL.md` w ten sposób:
...
queryai wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `queryai.json`:
...
**Dla agentów wbudowanych** (w `queryai.json`):
