...
Dzięki queryai możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie queryai używa naszego własnego motywu `queryai`.
...
queryai ma kilka wbudowanych motywów.
...
- Chcą, aby QueryAI pasował do wyglądu ich terminala
...
queryai obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/queryai/themes/*.json` lub `$XDG_CONFIG_HOME/queryai/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.queryai/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.queryai/themes/*.json`
