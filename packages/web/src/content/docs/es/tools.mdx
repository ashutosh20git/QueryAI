...
Las herramientas permiten que LLM realice acciones en su código base. QueryAI viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en QueryAI.
...
:::note
Esta herramienta solo está disponible cuando `QUERYAI_EXPERIMENTAL_LSP_TOOL=true` (o `QUERYAI_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor QueryAI o cuando la variable de entorno `QUERYAI_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar QueryAI:
