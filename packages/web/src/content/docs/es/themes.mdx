...
Con QueryAI puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, QueryAI usa nuestro propio tema `queryai`.
...
QueryAI viene con varios temas integrados.
...
- Quiere que QueryAI coincida con la apariencia de su terminal
...
QueryAI admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/queryai/themes/*.json` o `$XDG_CONFIG_HOME/queryai/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.queryai/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.queryai/themes/*.json`
