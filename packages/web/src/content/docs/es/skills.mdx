...
Las habilidades del agente permiten a QueryAI descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
QueryAI busca estas ubicaciones:

- Configuración del proyecto: `.queryai/skills/<name>/SKILL.md`
- Configuración global: `~/.config/queryai/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, QueryAI sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.queryai/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/queryai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.queryai/skills/git-release/SKILL.md` así:
...
QueryAI enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `queryai.json`:
...
**Para agentes integrados** (en `queryai.json`):
