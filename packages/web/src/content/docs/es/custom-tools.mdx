...
description: Cree herramientas que LLM pueda llamar en queryai.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de queryai como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.queryai/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/queryai/tools/`.
...
```ts title=".queryai/tools/database.ts" {1}
...
```ts title=".queryai/tools/math.ts"
import { tool } from "@queryai/plugin"
...
```ts title=".queryai/tools/bash.ts"
import { tool } from "@queryai/plugin"
...
```ts title=".queryai/tools/project.ts" {8}
import { tool } from "@queryai/plugin"
...
```python title=".queryai/tools/add.py"
import sys
...
```ts title=".queryai/tools/python-add.ts" {10}
import { tool } from "@queryai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".queryai/tools/add.py")
