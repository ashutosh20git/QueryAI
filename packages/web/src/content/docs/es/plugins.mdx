---
title: Complementos
description: Escriba sus propios complementos para extender QueryAI.
---

Los complementos le permiten extender QueryAI al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de QueryAI.
...
- `.queryai/plugins/` - Complementos a nivel de proyecto
- `~/.config/queryai/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/queryai/node_modules/`.
...
1. Configuración global (`~/.config/queryai/queryai.json`)
2. Configuración del proyecto (`queryai.json`)
3. Directorio global de complementos (`~/.config/queryai/plugins/`)
4. Directorio de complementos del proyecto (`.queryai/plugins/`)
...
```json title=".queryai/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
QueryAI ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".queryai/plugins/my-plugin.ts"
...
```js title=".queryai/plugins/example.js"
...
- `client`: Un cliente SDK queryai para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar queryai.
...
```js title=".queryai/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "queryai"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio QueryAI, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que queryai lea archivos `.env`:

```javascript title=".queryai/plugins/env-protection.js"
...
```javascript title=".queryai/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a queryai:

```ts title=".queryai/plugins/custom-tools.ts"
import { type Plugin, tool } from "@queryai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que queryai puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para queryai junto con las herramientas integradas.
...
```ts title=".queryai/plugins/compaction.ts"
import type { Plugin } from "@queryai/plugin"
...
```ts title=".queryai/plugins/custom-compaction.ts"
import type { Plugin } from "@queryai/plugin"
