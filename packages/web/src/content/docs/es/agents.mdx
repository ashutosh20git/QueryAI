...
Hay dos tipos de agentes en QueryAI; agentes primarios y subagentes.
...
QueryAI viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
QueryAI viene con tres subagentes integrados, **General**, **Explore** y **Scout**. Veremos esto a continuación.

---

## Integrados

QueryAI viene con dos agentes primarios integrados y tres subagentes integrados.
...
Configure agentes en su archivo de configuración `queryai.json`:
...
Si no se especifica ninguna temperatura, QueryAI utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global QueryAI como para la configuración específica del proyecto.
...
El ID del modelo en su configuración QueryAI usa el formato `provider/model-id`. Por ejemplo, si usa [QueryAI Zen](/docs/zen), usaría `queryai/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Ejecute `queryai models` para ver una lista de los modelos disponibles.
:::
