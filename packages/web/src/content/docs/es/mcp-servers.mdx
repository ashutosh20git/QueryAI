...
Puede agregar herramientas externas a QueryAI utilizando el _Model Context Protocol_ o MCP. QueryAI admite servidores locales y remotos.
...
Puede definir servidores MCP en su [QueryAI Config](https://opencode.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/queryai`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
QueryAI maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, QueryAI:
...
Si el servidor requiere autenticación, QueryAI le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `queryai mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, QueryAI almacenará los tokens de forma segura en `~/.local/share/queryai/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en QueryAI, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración QueryAI como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar QueryAI a su cuenta Sentry.
