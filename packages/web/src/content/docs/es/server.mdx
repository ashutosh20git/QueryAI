---
title: Servidor
description: Interactuar con el servidor queryai a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `queryai serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente queryai puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `queryai.local` |
...
Configure `QUERYAI_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `queryai`, o configure `QUERYAI_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `queryai serve` como a `queryai web`.
...
Cuando ejecuta `queryai`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor queryai para interactuar con queryai mediante programación.
:::

Esta arquitectura permite que queryai admita múltiples clientes y le permite interactuar con queryai mediante programación.

Puede ejecutar `queryai serve` para iniciar un servidor independiente. Si tienes el
queryai TUI ejecutándose, `queryai serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos QueryAI [IDE](/docs/ide).
...
El servidor queryai expone las siguientes API.
