---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um QueryAI zu erweitern.
---

Mit Plugins können Sie QueryAI erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von QueryAI zu ändern.
...
- `.queryai/plugins/` – Plugins auf Projektebene
- `~/.config/queryai/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/queryai/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/queryai/queryai.json`)
2. Projektkonfiguration (`queryai.json`)
3. Globales Plugin-Verzeichnis (`~/.config/queryai/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.queryai/plugins/`)
...
QueryAI führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein QueryAI-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie QueryAI erweitern können.
...
:::note
Wenn Sie die QueryAI-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass QueryAI `.env`-Dateien liest:
...
Plugins können QueryAI auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das QueryAI aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für QueryAI zur Verfügung.
