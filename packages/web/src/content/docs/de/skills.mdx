...
Agent Skills erlauben QueryAI, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
QueryAI sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft QueryAI vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.queryai/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/queryai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.queryai/skills/git-release/SKILL.md` zum Beispiel so:
...
QueryAI listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `queryai.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `queryai.json`):
