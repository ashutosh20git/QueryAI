---
title: Regeln
description: Lege eigene Anweisungen fuer queryai fest.
---

Du kannst queryai ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in queryai den Befehl `/init` aus.
...
So kann queryai sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht queryai sie zu erweitern.
...
queryai kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/queryai/AGENTS.md` speichern.
Diese gelten in allen queryai-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt QueryAI dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/queryai/AGENTS.md` existiert)
...
Beim Start sucht queryai in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/queryai/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `queryai.json` oder in `~/.config/queryai/queryai.json` angeben.
...
queryai parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit queryai.json

Empfohlen ist das `instructions`-Feld in der `queryai.json`:
...
Du kannst queryai explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `queryai.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
