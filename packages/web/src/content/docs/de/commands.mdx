...
Erstellen Sie `.queryai/commands/test.md`:

```md title=".queryai/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die QueryAI-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem QueryAI [config](/docs/config):

```json title="queryai.jsonc" {4-12}
{
...
- Global: `~/.config/queryai/commands/`
- Pro Projekt: `.queryai/commands/`

```markdown title="~/.config/queryai/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".queryai/commands/component.md"
---
...
```md title=".queryai/commands/create-file.md"
---
...
```md title=".queryai/commands/analyze-coverage.md"
---
...
```md title=".queryai/commands/review-changes.md"
---
...
```md title=".queryai/commands/review-component.md"
---
...
QueryAI enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
