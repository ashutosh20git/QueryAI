...
Mit QueryAI kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet QueryAI das `queryai`-Theme.
...
QueryAI liefert mehrere eingebaute Themes mit.
...
- QueryAI optisch ans Terminal anpassen wollen
...
QueryAI unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/queryai/themes/*.json` oder `$XDG_CONFIG_HOME/queryai/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.queryai/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.queryai/themes/*.json`
