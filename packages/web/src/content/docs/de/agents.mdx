...
Es gibt zwei Arten von Agenten in QueryAI; Primäragenten und Subagenten.
...
QueryAI verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `queryai.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet QueryAI modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale QueryAI-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer QueryAI-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [QueryAI Zen](/docs/zen) verwenden, würden Sie `queryai/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `queryai models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
