...
Wenn QueryAI Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\queryai\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `queryai --log-level DEBUG`.

---

## Speicher

queryai speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/queryai/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\queryai`
...
QueryAI Desktop startet im Hintergrund einen lokalen QueryAI-Server (`queryai-cli`-Sidecar).
...
- Nur macOS: `QueryAI`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/queryai/queryai.jsonc` (oder `~/.config/queryai/queryai.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/queryai/queryai.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\queryai\queryai.jsonc`
...
QueryAI kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/queryai/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\queryai\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit QueryAI ihn neu aufbauen kann.

1. Beende QueryAI Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/queryai`
- **Linux**: loesche `~/.cache/queryai` (oder fuehre aus: `rm -rf ~/.cache/queryai`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\queryai`

3. Starte QueryAI Desktop neu.

---

### Server-Verbindungsprobleme beheben

QueryAI Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `queryai.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `QUERYAI_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `QUERYAI_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt QueryAI Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit QueryAI-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

QueryAI Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer QueryAI in den OS-Einstellungen aktiviert sind, und
...
1. Beende QueryAI Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von QueryAI Desktop):

- `queryai.settings.dat` (desktop default server URL)
- `queryai.global.dat` und `queryai.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit QueryAI hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `queryai models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\queryai`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. queryai installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\queryai`

2. Starte queryai neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt queryai, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
queryai erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
