---
title: Server
description: Nutze den queryai-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `queryai serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein queryai-Client nutzen kann.
...
Setze `QUERYAI_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `queryai`, kann aber mit `QUERYAI_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `queryai serve` und `queryai web`.
...
Wenn du `queryai` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den queryai-Server, um queryai programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `queryai serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `queryai serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die QueryAI-[IDE](/docs/ide)-Plugins.
...
Der queryai-Server stellt folgende APIs bereit.
