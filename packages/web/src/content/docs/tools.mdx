...
Tools allow the LLM to perform actions in your codebase. QueryAI comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in QueryAI.
...
:::note
This tool is only available when `QUERYAI_EXPERIMENTAL_LSP_TOOL=true` (or `QUERYAI_EXPERIMENTAL=true`).
:::
...
:::note
Available by default, backed by a file on your machine. Set `MEM0_API_KEY` or a `memory.api_key` to store memories in mem0 instead, or `"memory": { "enabled": false }` to turn the tool off. [Learn more](/docs/config/#memory)
:::

```json title="queryai.json" {4-9}
{
  "$schema": "https://raw.githubusercontent.com/ashutosh20git/QueryAI/schema/config.json",
  "permission": {
    "memory": {
...
Supports four actions, which double as the permission patterns: `remember` saves
a fact, `search` looks one up, `list` shows what is stored, and `forget` deletes
one by id. Facts are saved with `project` scope by default; `user` scope is for
things true of you everywhere, and is recalled in every project.
...
:::note
This tool is only available when using the QueryAI or QueryAI Go provider, or when either the `QUERYAI_ENABLE_EXA` or `QUERYAI_ENABLE_PARALLEL` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching QueryAI:

```bash
QUERYAI_ENABLE_EXA=1 queryai
# or
QUERYAI_ENABLE_PARALLEL=1 queryai
```
