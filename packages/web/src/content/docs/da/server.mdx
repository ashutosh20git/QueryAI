---
title: Server
description: Interager med queryai-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `queryai serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en queryai-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `queryai.local` |
...
Indstil `QUERYAI_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `queryai`, eller indstil `QUERYAI_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `queryai serve` og `queryai web`.
...
Når du kører `queryai` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug queryai-serveren til at interagere med queryai programmatisk.
:::

Denne arkitektur lader queryai understøtte flere klienter og lader dig interagere med queryai programmatisk.

Du kan køre `queryai serve` for at starte en selvstændig server. Hvis du har
queryai TUI kørende, vil `queryai serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af QueryAI [IDE](/docs/ide) plugins.
...
QueryAI-serveren viser følgende API'er.
