...
QueryAI Go er et lavprisabonnement til **$10/måned**, der giver dig pålidelig adgang til populære åbne kodningsmodeller.

Go fungerer som enhver anden udbyder i QueryAI. Du abonnerer på QueryAI Go og
...
:::tip
Vi testede en udvalgt gruppe af modeller og udbydere, der fungerer godt med QueryAI.
:::
...
QueryAI Go giver dig adgang til disse modeller for **$10/måned**.

---

## Sådan fungerer det

QueryAI Go fungerer som enhver anden udbyder i QueryAI.

1. Du logger ind på **<a href={console}>QueryAI Zen</a>**, abonnerer på Go, og
   kopierer din API-nøgle.
2. Du kører kommandoen `/connect` i TUI'en, vælger `QueryAI Go`, og indsætter
   din API-nøgle.
3. Kør `/models` i TUI'en for at se listen over tilgængelige modeller gennem Go.

:::note
Kun ét medlem per arbejdsområde kan abonnere på QueryAI Go.
:::
...
QueryAI Go inkluderer følgende grænser:
...
Dit [model id](/docs/config/#models) i din QueryAI config
bruger formatet `queryai-go/<model-id>`. For eksempel for Kimi K3, vil du
bruge `queryai-go/kimi-k3` i din config.
...
4. Undgå **lock-in** ved også at lade dig bruge enhver anden udbyder med QueryAI.
