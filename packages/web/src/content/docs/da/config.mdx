---
title: Konfiguration
description: Ved at bruge QueryAI JSON konfig.
---

Du kan konfigurere QueryAI ved hjælp af en JSON-konfigurationsfil.

---

## Format

QueryAI understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/queryai`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/queryai/queryai.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`QUERYAI_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`queryai.json` i projekt) - projektspecifikke indstillinger
5. **`.queryai` mapper** - agent, kommandoer, plugins
6. **Inline config** (`QUERYAI_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.queryai` og `~/.config/queryai` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/queryai`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale QueryAI-konfiguration i `~/.config/queryai/queryai.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `queryai.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når QueryAI starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `QUERYAI_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `QUERYAI_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.queryai` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.queryai` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`opencode.ai/config.json`**](https://raw.githubusercontent.com/ashutosh20git/QueryAI-dist/schema/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `queryai serve` og `queryai web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din QueryAI-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `queryai.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din QueryAI-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger QueryAI at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din QueryAI-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/queryai/agents/` eller `.queryai/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil QueryAI falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`queryai run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/queryai/commands/` eller `.queryai/commands/`. [Learn more here](/docs/commands).
...
QueryAI vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard queryai **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide QueryAI med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.queryai/plugins/` eller `~/.config/queryai/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse QueryAI til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
