...
Med QueryAI kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger QueryAI vårt eget `queryai`-tema.
...
QueryAI støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/queryai/themes/*.json` eller `$XDG_CONFIG_HOME/queryai/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.queryai/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.queryai/themes/*.json`
