...
Her er alle de innebygde verktøyene som er tilgængelige i QueryAI.
...
:::note
Dette verktøyet er bare tilgængelig når `QUERYAI_EXPERIMENTAL_LSP_TOOL=true` (eller `QUERYAI_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger QueryAI-leverandøren eller når miljøvariabelen `QUERYAI_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
