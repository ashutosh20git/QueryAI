...
Der er to typer agenter i QueryAI; primære agenter og subagenter.
...
QueryAI leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
QueryAI leveres med tre indbyggede subagenter, **General**, **Explore** og **Scout**. Vi ser nærmere på dem nedenfor.

---

## Indbyggede

QueryAI leveres med to indbyggede primære agenter og tre indbyggede subagenter.
...
En skrivebeskyttet agent til eksterne docs og research af dependencies. Brug denne, når du har brug for at klone et dependency-repository ind i QueryAI's administrerede cache, inspicere kildekoden i et bibliotek eller krydstjekke lokal kode mod upstream-implementeringer uden at ændre dit workspace.
...
Konfigurer agent i din `queryai.json`-konfigurationsfil:
...
- Globalt: `~/.config/queryai/agents/`
- Pr. projekt: `.queryai/agents/`
...
Hvis ingen temperatur er angivet, bruger QueryAI modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale QueryAI-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din QueryAI-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [QueryAI Zen](/docs/zen), vil du bruge `queryai/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `queryai models` for at se en liste over de tilgængelige modeller.
:::
