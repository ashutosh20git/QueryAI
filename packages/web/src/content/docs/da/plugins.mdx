---
title: Plugins
description: Skriv dine egne plugins for at udvide QueryAI.
---

Plugins giver dig mulighed for at udvide QueryAI ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre QueryAIs standardadfærd.
...
- `.queryai/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/queryai/node_modules/`.
...
1. Global konfiguration (`~/.config/queryai/queryai.json`)
2. Projektkonfiguration (`queryai.json`)
3. Global plugin-mappe (`~/.config/queryai/plugins/`)
4. Projekt plugin bibliotek (`.queryai/plugins/`)
...
QueryAI kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En queryai SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide queryai.
...
:::note
Hvis du bruger QueryAI desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå queryai i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til queryai:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som queryai kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for queryai sammen med indbyggede værktøjer.
