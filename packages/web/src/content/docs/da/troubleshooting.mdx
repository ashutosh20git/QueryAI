...
For at fejlfinde problemer med QueryAI, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\queryai\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `queryai --log-level DEBUG`.

---

## Lagring

queryai gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/queryai/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\queryai`
...
QueryAI Desktop kører en lokal QueryAI-server (`queryai-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `QueryAI`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/queryai/queryai.jsonc` (eller `~/.config/queryai/queryai.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/queryai/queryai.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\queryai\queryai.jsonc`
...
QueryAI kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/queryai/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\queryai\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så QueryAI kan genopbygge den.

1. Afslut QueryAI Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/queryai`
- **Linux**: slet `~/.cache/queryai` (eller kør `rm -rf ~/.cache/queryai`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\queryai`

3. Start QueryAI Desktop på ny.

---

### Løs problemer med serverforbindelse

QueryAI Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `queryai.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `QUERYAI_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `QUERYAI_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver QueryAI Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i QueryAI.

---

### Meddelelser vises ikke

QueryAI Desktop viser kun systemvarsler når:

- varsler er aktiveret for QueryAI i dine OS-indstillinger, og
...
1. Afslut QueryAI Desktop.
2. Find og slet disse filer (de findes i QueryAI Desktop-appens datamappe):

- `queryai.settings.dat` (skrivebordsstandardserver URL)
- `queryai.global.dat` og `queryai.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med QueryAI:
...
For at finde ud af hvilke modeller du har adgang til, kør `queryai models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\queryai`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. queryai installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\queryai`

2. Start queryai på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge queryai til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
queryai vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
