...
Agentferdigheter lar QueryAI oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
queryai søker etter disse stedene:

- Prosjektkonfigurasjon: `.queryai/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/queryai/skills/<name>/SKILL.md`
...
For prosjektlokale stier går queryai opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.queryai/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/queryai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.queryai/skills/git-release/SKILL.md` slik:
...
queryai viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `queryai.json`:
...
**For innebygde agenter** (i `queryai.json`):
