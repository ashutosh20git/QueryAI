...
Det er to typer agenter i QueryAI; primære agenter og underagenter.
...
QueryAI kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
QueryAI kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

QueryAI kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `queryai.json`:
...
Hvis ingen temperatur er spesifisert, bruker QueryAI modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale QueryAI-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i QueryAI-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [QueryAI Zen](/docs/zen), vil du bruke `queryai/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `queryai models` for å se en liste over tilgjengelige modeller.
:::
