---
title: TUI
description: Bruke queryai-terminalbrukergrensesnittet.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

QueryAI gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre QueryAI starter TUI for gjeldende katalog.
...
Når du bruker QueryAI TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til QueryAI. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt QueryAI. _Aliaser_: `/quit`, `/q`
...
Dette er atskilt fra `queryai.json`, som konfigurerer server-/kjøretidsoppførsel.
...
- `leader_timeout` - Styrer hvor lenge QueryAI venter etter leader key. Standard er `2000`.
...
Bruk `QUERYAI_TUI_CONFIG` for å laste en egendefinert TUI-konfigurasjonsbane.
...
- `sound_pack` - Sound pack-ID som skal brukes. Standard er `queryai.default`.
