...
description: Skriv dine egne programtillegg for å utvide QueryAI.
---

Plugins lar deg utvide QueryAI ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til QueryAI.
...
- `.queryai/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/queryai/node_modules/`.
...
1. Global konfigurasjon (`~/.config/queryai/queryai.json`)
2. Prosjektkonfigurasjon (`queryai.json`)
3. Global plugin-katalog (`~/.config/queryai/plugins/`)
4. Prosjektpluginkatalog (`.queryai/plugins/`)
...
QueryAI kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En QueryAI SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide QueryAI.
...
:::note
Hvis du bruker QueryAI-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre QueryAI fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til QueryAI:
...
`tool`-hjelperen lager et tilpasset verktøy som QueryAI kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for QueryAI sammen med innebygde verktøy.
