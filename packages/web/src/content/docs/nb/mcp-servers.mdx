...
Du kan legge til eksterne verktøy i QueryAI ved å bruke _Model Context Protocol_ eller MCP. QueryAI støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [QueryAI Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="queryai.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/queryai`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
QueryAI håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil QueryAI:
...
Hvis serveren krever autentisering, vil QueryAI be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `queryai mcp auth <server-name>`.
...
```bash
queryai mcp auth my-oauth-server
```
...
```bash
queryai mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil QueryAI lagre tokens sikkert i `~/.local/share/queryai/mcp-auth.json`.
...
queryai mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i QueryAI, sammen med innebygde verktøy. Så du kan administrere dem gjennom QueryAI-konfigurasjonen som alle andre verktøy.
...
```json title="queryai.json" {11, 14-18}
{
  "$schema": "https://raw.githubusercontent.com/ashutosh20git/QueryAI-dist/schema/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
queryai mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble QueryAI til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver QueryAI-samtale oppført som en separat økt.
