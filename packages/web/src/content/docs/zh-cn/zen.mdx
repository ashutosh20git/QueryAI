---
title: Zen
description: 由 QueryAI 提供的精选模型列表。
---
...
QueryAI Zen 是由 QueryAI 团队提供的一组经过测试和验证的模型。

Zen 的工作方式与 QueryAI 中的任何其他提供商相同。你登录 QueryAI Zen 并获取 API 密钥。它是**完全可选的**，即使不用它，你也可以照常使用 QueryAI。
...
:::tip
我们测试了一组与 QueryAI 配合良好的精选模型和提供商。
:::
...
QueryAI Zen 是一个 AI 网关，让你可以访问这些模型。

---

## 工作原理

QueryAI Zen 的工作方式与 QueryAI 中的任何其他提供商相同。

1. 登录 **<a href={console}>QueryAI Zen</a>**，添加你的账单信息，然后复制你的 API 密钥。
2. 在 TUI 中运行 `/connect` 命令，选择 QueryAI Zen，然后粘贴你的 API 密钥。
...
在你的 QueryAI 配置中，[模型 ID](/docs/config/#models) 使用 `queryai/<model-id>` 格式。例如，对于 GPT 5.5，你需要在配置中使用 `queryai/gpt-5.5`。
...
你可能会在使用记录中看到 Haiku、Nano 或 Flash 等[低成本模型](/docs/config/#models)。QueryAI 使用这些模型生成会话标题。
...
- MiMo-V2.5 Free 目前在 QueryAI 上限时免费提供。团队正在利用这段时间收集反馈并改进模型。
- Ling 3.0 Flash Fin Free 目前在 QueryAI 上限时免费提供。团队正在利用这段时间收集反馈并改进模型。
- Nemotron 3 Ultra Free 目前在 QueryAI 上限时免费提供。团队正在利用这段时间收集反馈并改进模型。
- Nemotron 3.5 Lightning Free 目前在 QueryAI 上限时免费提供。团队正在利用这段时间收集反馈并改进模型。
- Big Pickle 是一个隐身模型，目前在 QueryAI 上限时免费提供。团队正在利用这段时间收集反馈并改进模型。
- Muse Spark 1.2 Contributor Free 目前在 QueryAI 上限时免费提供。团队正在利用这段时间收集反馈并改进模型。
