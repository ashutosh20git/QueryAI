...
Create `.queryai/commands/test.md`:
...
You can add custom commands through the QueryAI config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your QueryAI [config](/docs/config):
...
queryai includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
