...
QueryAI uses the `permission` config to decide whether a given action should run automatically, prompt you, or be blocked.
...
Start QueryAI with `--auto` to automatically approve permission requests that are not explicitly denied.

```bash
queryai --auto
```

You can also use auto mode with [`queryai run`](/docs/cli#run).

```bash
queryai run --auto "Refactor this module"
```
...
Use `external_directory` to allow tool calls that touch paths outside the working directory where QueryAI was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `glob`, `grep`, and many `bash` commands).
...
QueryAI permissions are keyed by tool name, plus a couple of safety guards:
...
- `memory` — reading and writing persistent memory (matches the action: `remember`, `search`, `list`, `forget`)
...
If you don’t specify anything, QueryAI starts from permissive defaults:
...
When QueryAI prompts for approval, the UI offers three outcomes:

- `once` — approve just this request
- `always` — approve future requests matching the suggested patterns (for the rest of the current QueryAI session)
