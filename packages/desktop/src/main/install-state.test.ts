...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier QueryAI launch", () => {
    expect(hasExistingAppState([file("queryai.settings")])).toBe(true)
    expect(hasExistingAppState([file("queryai.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("queryai")])).toBe(true)
  })
})
