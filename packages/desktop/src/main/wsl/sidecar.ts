...
import { type WslCommandLine, resolveWslQueryAI, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const queryai = await resolveWslQueryAI(distro)
  if (!queryai) throw new Error(nativeT("desktop.wsl.error.queryaiNotInstalled", { distro }))

  const port = await allocatePort()
  const password = randomUUID()
  const username = "queryai"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export QUERYAI_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export QUERYAI_CLIENT=desktop",
    `export QUERYAI_SERVER_USERNAME=${shellEscape(username)}`,
    `export QUERYAI_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(queryai)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
