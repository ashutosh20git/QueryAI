...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.queryai.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `queryai.settings.dat` is special: it maps to the `queryai.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"queryai.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // queryai.settings.dat → the electron settings store ("queryai.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "queryai.global.dat").
  const storeName = filename === "queryai.settings.dat" ? "queryai.settings" : filename
